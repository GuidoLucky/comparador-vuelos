...
async function cronMonitorTLV() {
  ...
  await Promise.all(TLV_FECHAS.map(async (fecha) => {
    await Promise.all(TLV_DESTINOS.map(async (dest) => {
      try {
        ...
        if (token) {
          ...
          if (r?.ok) {
            ...
            const vuelos = procesarVuelos(data, 0); // 0 = solo directos
        ...
        const llToken = await getUserLleegoToken(1).catch(() => null); // usuario Guido
        if (llToken) {
          ...
          if (lr?.ok) {
            ...
            const llVuelos = buscarLleego ? [] : []; // GEA devuelve token para polling
            // Para GEA usar searchToken si lo devuelve
            const searchToken = llData?.search_token || llData?.token;
            if (searchToken) {
              await new Promise(r => setTimeout(r, 4000)); // esperar resultados
