      ...
      try {
        ...
        if (llToken) {
          ...
          if (lr?.ok) {
            ...
            if (filtGEA.length) {
              const aerolineasVistas = new Set();
              filtGEA.forEach(sol => {
                const assocs = sol.data?.associations || [];
                assocs.forEach(assoc => {
                  (assoc.journey_references || []).forEach(jRef => {
                    const journey = journeys[jRef];
                    (journey?.segments || []).forEach(segId => {
                      const seg = segments[segId];
                      if (seg?.marketing_company) aerolineasVistas.add(seg.marketing_company);
                    });
                  });
                });
              });
              encontrados.push({
                ...
                vuelos: [{ aerolinea: [...aerolineasVistas].join('/'), precio: filtGEA[0]?.total_price?.total || 0 }]
              });
            }
          } else {
...
async function enviarEmailTLV(encontrados) {
  try {
    ...
    for (const [fecha, items] of Object.entries(porFecha)) {
      ...
      for (const e of items) {
        const vuelosStr = e.vuelos.map(v => {
          ...
          const waMensaje = encodeURIComponent(`Hola, quiero este vuelo TLV → ${e.destino.iata} (${e.destino.nombre}) del ${fechaFmt}. Aerolínea: ${v.aerolinea}. Necesito X cantidad de lugares. Por favor reservar. Te envío la foto del pasaporte.`);
          ...
          return `
            <div style="display:flex;justify-content:space-between;align-items:center;padding:6px 0;border-bottom:1px solid #1e1e2e">
              <span style="color:#aaa;font-size:12px">${v.aerolinea}${v.salida ? ` — Salida: ${v.salida.substring(11,16)}` : ''}</span>
              <div style="display:flex;align-items:center;gap:10px">
                ...
                <a href="${waUrl}" style="background:#25D366;color:#fff;font-size:11px;font-weight:700;padding:5px 10px;border-radius:6px;text-decoration:none;white-space:nowrap">💬 Reservar por WA</a>
              </div>
            </div>`;
        }).join('');
        html += `
    ...
    html += `
        <p style="color:#555;font-size:11px;margin-top:16px;text-align:center">Monitor TLV — Lucky Tour · Se actualiza cada 3 minutos</p>
        </div>
      </div>`;
    ...
    await resendClient.emails.send({
      ...
      to: TLV_EMAIL_TO,
      subject: `🚨 TLV Disponibilidad — ${encontrados.length} ruta${encontrados.length !== 1 ? 's' : ''} con lugares`,
      html
    });
    console.log(`[TLV] Email enviado a ${TLV_EMAIL_TO.join(', ')}`);
  } catch(e) {
