...
async function getUserLleegoToken(userId) {
  ...
  const agent = extra.agent || LLEEGO_AGENT;
  
  const url = `https://middle.lleego.com/api/user/auto-login?e=${encodeURIComponent(cred.cred_user)}&w=${encodeURIComponent(cred.cred_pass)}&idAgente=${encodeURIComponent(agent)}`;
