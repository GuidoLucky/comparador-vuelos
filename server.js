...
function procesarVuelosLleego(resp, paxCounts = { adultos: 1, ninos: 0, infantes: 0 }) {
  ...
  return solutions.flatMap((sol, solIdx) => {
    try {
      ...
      // Deduplicar asociaciones por combinación única de journey_references
      // Asociaciones con los mismos vuelos pero distinto proveedor/validating son duplicados
      const assocsPorJourneys = new Map();
      for (const assoc of assocsFull) {
        const key = (assoc.journey_references || []).sort().join('|');
        if (!assocsPorJourneys.has(key)) {
          assocsPorJourneys.set(key, assoc);
        }
      }
      const assocsUnicos = Array.from(assocsPorJourneys.values());

      const opcionesPorAssoc = assocsUnicos.length > 0 ? assocsUnicos.map((_, assocIdx) => assocIdx) : [0];

      return opcionesPorAssoc.map(assocIdx => {
      const assocActual = assocsUnicos[assocIdx] || assocsFull[0];
      // Cache: guardar sol con solo la asociación correspondiente
      const solParaCache = { ...sol, data: { ...sol.data, associations: [assocActual] } };
      const cacheKey = assocsUnicos.length > 1 ? `lleego_${sol.id}_a${assocIdx}` : `lleego_${sol.id}`;
      ...
      // También cachear por id base para compatibilidad (primera asociación)
      if (assocIdx === 0) lleegoSolutionsCache.set(`lleego_${sol.id}`, { sol: solParaCache, segments, journeys, fares, companies, providers, ports, searchToken, paxCounts });
      
      // Log first solution structure for debugging
      if (solIdx === 0 && assocIdx === 0) {
        ...
        // Log all associations and their journey_references
        console.log(`[Lleego] Sol[0] total assocs: ${assocsFull.length}, unique: ${assocsUnicos.length}`);
        assocsFull.forEach((a, i) => console.log(`[Lleego] Sol[0] assoc[${i}] journeys:`, a.journey_references));
      }
