  ...
  try {
    ...
    const r = await db.query(`
      SELECT id, pnr, order_id, estado, gds, notas, fecha_salida, aerolinea, usuario_id,
             origen, destino, itinerario_json, adultos, ninos, infantes,
    ...
    for (const reserva of r.rows) {
      try {
        ...
        const vuelosParaComparar = isGEA
          ...
          : (detalle?.vuelos || []);
        if (vuelosParaComparar.length && reserva.itinerario_json) {
          const itinGuardado = typeof reserva.itinerario_json === 'string'
            ? JSON.parse(reserva.itinerario_json) : reserva.itinerario_json;
          cambiosItinerario = detectarCambiosItinerario(itinGuardado, vuelosParaComparar);
        }
...
function normalizarVuelo(fn) {
  ...
  // Separar letras (aerolinea) de números
  const m = s.match(/^([A-Z]{2,3})(\d+)$/);
  if (m) return m[1] + String(parseInt(m[2], 10)); // quita ceros: MU0710 → MU710
  return s;
}

// Compara itinerario guardado en DB vs vuelos actuales de la API
// Compara POR NÚMERO DE VUELO, no por posición, para manejar conexiones correctamente
function detectarCambiosItinerario(itinerarioGuardado, vuelosAPI) {
  const cambios = [];
  if (!itinerarioGuardado || !vuelosAPI?.length) return cambios;

  // Aplanar segmentos del itinerario guardado
  const segsGuardados = [];
  ...
  // Construir mapa de segmentos guardados por número de vuelo normalizado
  // Si hay duplicado (vuelo con misma numeración en ida y vuelta), guardamos array
  const mapaGuardados = {};
  ...
  // Rastrear qué segmentos guardados ya fueron matcheados (para detectar cancelados reales)
  ...
  for (const v of vuelosAPI) {
    ...
    const segLabel     = `${flightNumRaw || flightNum} ${depIATA}→${arrIATA}`;

    // Vuelo cancelado en API — solo reportar si lo teníamos guardado
    const statusCancelado = ['XX','HX','UC','UN','NO'].includes(v.status);
    if (statusCancelado) {
      if (flightNum && mapaGuardados[flightNum]) {
        cambios.push({ tipo: 'CANCELADO', segmento: segLabel, status: v.status });
      }
      continue;
    }

    // Buscar segmento guardado que coincida con este vuelo
    // Primero por número de vuelo exacto, luego por origen+destino como fallback
    let g = null;
    const candidatos = mapaGuardados[flightNum] || [];
    if (candidatos.length === 1) {
      g = candidatos[0];
    } else if (candidatos.length > 1) {
      // Mismo vuelo en ida y vuelta: elegir el que tiene origen más parecido
      g = candidatos.find(c => (c.origen || c.departureAirportCode || '') === depIATA) || candidatos[0];
    } else {
      // No encontramos por número de vuelo → buscar por par origen+destino
      g = segsGuardados.find(s =>
        (s.origen || s.departureAirportCode || '') === depIATA &&
        (s.destino || s.arrivalAirportCode   || '') === arrIATA
      );
      // Si tampoco → este vuelo no estaba antes, no es un "cambio" sino posiblemente segmento nuevo
      // Solo reportarlo si hay muy pocos segmentos guardados (evitar falsos positivos)
      if (!g) {
        console.log(`[Cron-Detect] Vuelo ${segLabel} no encontrado en guardados, skip`);
        continue;
      }
    }

    const gKey = normalizarVuelo(g.vuelo || g.flightNumber || '') + '_' + (g.salida || '');
    matcheados.add(gKey);

    const depG = g.salida || g.departureDateTime || '';

    // Cambio de horario (cualquier diferencia mayor a 5 minutos se reporta)
    if (depAPI && depG) {
      const tAPI = new Date(depAPI).getTime();
      const tG   = new Date(depG).getTime();
      if (!isNaN(tAPI) && !isNaN(tG) && Math.abs(tAPI - tG) > 5 * 60 * 1000) {
        const diffMin = Math.round((tAPI - tG) / 60000);
        cambios.push({ tipo: 'HORARIO', segmento: segLabel, diffMin, original: depG, nuevo: depAPI });
      }
    }

    // Cambio de aeropuerto (solo si teníamos dato guardado)
    ...
    // Cambio de número de vuelo (solo si el vuelo NO se encontró por número — lo encontramos por ruta)
    if (!mapaGuardados[flightNum] && flightNum) {
      const vueloG = g.vuelo || g.flightNumber || '';
      if (vueloG && normalizarVuelo(vueloG) !== flightNum) {
        cambios.push({ tipo: 'VUELO', segmento: segLabel, original: vueloG, nuevo: flightNum });
      }
    }
  }
