...
async function cronMonitorTLV() {
  ...
  for (let i = 0; i < combinaciones.length; i += BATCH) {
    ...
    await Promise.all(batch.map(async ({ fecha, dest }) => {
      try {
        // Buscar en Tucano (GLAS)
        const token = await getToken().catch(() => null);
        if (token) {
          console.log(`[TLV] Tucano buscando TLV→${dest.iata} ${fecha}...`);
          const payload = {
            DepartCode: 'TLV', ArrivalCode: dest.iata,
            DepartDate: `${fecha}T00:00:00`, DepartTime: null,
            Adults: 1, Childs: 0, Infants: 0,
            CabinType: null, Stops: 0, Airlines: [],
          ...
          const r = await fetch(`${API_BASE}/FlightSearch/OnewayRemake`, {
            method: 'POST', headers: getHeaders(token),
            body: JSON.stringify(payload)
          }).catch(() => null);
          if (r?.ok) {
            const data = await r.json().catch(() => null);
            const vuelos = procesarVuelos(data, 0);
            console.log(`[TLV] Tucano TLV→${dest.iata} ${fecha}: ${vuelos.length} vuelos totales`);
            const filtrados = vuelos.filter(v => {
              if (v.escalas !== 0) return false;
              const carrier = (v.aerolinea || '').toUpperCase();
              if (TLV_AEROLINEAS.includes(carrier)) return true;
              const segs = v.itinerario?.flatMap(l => l.segmentos || []) || [];
              return segs.some(s => TLV_AEROLINEAS.includes((s.aerolinea || s.vuelo?.substring(0,2) || '').toUpperCase()));
            });
            console.log(`[TLV] Tucano TLV→${dest.iata} ${fecha}: ${filtrados.length} con IZ/IS/LY`);
            if (filtrados.length) {
              encontrados.push({
                fecha, destino: dest, fuente: 'Tucano',
                cantidad: filtrados.length,
                vuelos: filtrados.slice(0, 3).map(v => ({
                  aerolinea: v.aerolinea,
                  precio: v.precioUSD,
                  salida: v.itinerario[0]?.salida,
                  llegada: v.itinerario[0]?.llegada,
                  escalas: v.escalas
                }))
              });
            }
          } else {
            console.log(`[TLV] Tucano TLV→${dest.iata} ${fecha}: sin respuesta (status ${r?.status || 'null'})`);
          }
        }

        // Buscar en GEA/Lleego
        ...
        if (llToken) {
          ...
          if (lr?.ok) {
            ...
            const sols = lData?.solutions || lData?.data?.solutions || [];
            console.log(`[TLV] GEA TLV→${dest.iata} ${fecha}: ${sols.length} soluciones totales`);
            const vuelosGEA = procesarVuelosLleego(lData, { adultos: 1, ninos: 0, infantes: 0 });
            const filtGEA = vuelosGEA.filter(v => {
              const carrier = (v.aerolinea || '').toUpperCase();
              const segs = v.itinerario?.flatMap(l => l.segmentos || []) || [];
              return TLV_AEROLINEAS.includes(carrier) || segs.some(s => TLV_AEROLINEAS.includes((s.aerolinea || '').toUpperCase()));
            });
            ...
            if (filtGEA.length) {
              encontrados.push({
                ...
                vuelos: filtGEA.slice(0, 3).map(v => ({
                  precio: v.precioUSD,
                  aerolinea: v.aerolinea
                }))
              });
  ...
  if (encontrados.length) {
    ...
    // await enviarEmailTLV(encontrados); // TODO: activar cuando confirmemos que son IZ/IS/LY
  } else {
    ...
    for (const [fecha, items] of Object.entries(porFecha)) {
      ...
      for (const e of items) {
        const vuelosStr = e.vuelos.map(v =>
          `<span style="color:#aaa">${v.aerolinea} — USD ${v.precio?.toFixed(2) || '?'}${v.salida ? ` — Salida: ${v.salida.substring(11,16)}` : ''}</span>`
        ).join('<br>');
        html += `
          <div style="background:#13131f;border:1px solid #333;border-left:3px solid #2ecc71;border-radius:6px;padding:10px 14px;margin-bottom:8px">
            <div style="display:flex;justify-content:space-between;align-items:center">
              ...
              <span style="background:#2ecc71;color:#000;font-size:11px;font-weight:700;padding:2px 8px;border-radius:10px">${e.cantidad} lugar${e.cantidad !== 1 ? 'es' : ''} — ${e.fuente}</span>
            </div>
            <div style="margin-top:8px;font-size:12px;line-height:1.8">${vuelosStr}</div>
          </div>`;
      }
    }
