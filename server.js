    ...
    await Promise.all(batch.map(async ({ fecha, dest }) => {
      try {
        ...
        const filtGEA = sols.filter(sol => {
          const assocs = sol.data?.associations || [];
          for (const assoc of assocs) {
            for (const jRef of (assoc.journey_references || [])) {
              const journey = journeys[jRef];
              if (!journey) continue;
            ...
            if (TLV_AEROLINEAS.includes((assoc.validating_company || '').toUpperCase())) return true;
          }
          return false;
        });
        ...
        // Obtener info del primer vuelo
        const sol = filtGEA[0];
        const assocs = sol.data?.associations || [];
        let vuelo = '', salida = '', llegada = '', seats = null;
        assocs.forEach(assoc => {
          (assoc.journey_references || []).forEach(jRef => {
            const journey = journeys[jRef];
            const segs = journey?.segments || [];
            if (segs.length && !vuelo) {
              const firstSeg = segments[segs[0]];
              const lastSeg = segments[segs[segs.length - 1]];
              if (firstSeg) {
                vuelo = `${firstSeg.marketing_company}${firstSeg.transport_number}`;
                salida = firstSeg.departure_date?.substring(11, 16) || '';
              }
              if (lastSeg) llegada = lastSeg.arrival_date?.substring(11, 16) || '';
              segs.forEach(segId => {
                const segRef = assoc.segment_references?.[segId];
                if (segRef?.seats != null && (seats === null || segRef.seats < seats)) seats = segRef.seats;
              });
            }
          });
        });

        const precioFinal = Math.ceil((sol.total_price?.total || 0) + 40);
