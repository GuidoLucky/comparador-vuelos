  ...
  try {
    ...
    let html = `
      <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto">
        <div style="background:#c9a84c;padding:16px 20px;border-radius:8px 8px 0 0">
    ...
    for (const [fecha, items] of Object.entries(porFecha)) {
      ...
      for (const e of items) {
        const vuelosStr = e.vuelos.map(v => {
          ...
          const vueloStr = v.vuelo ? `${v.vuelo}` : '';
          ...
          return `
            <div style="display:flex;justify-content:space-between;align-items:center;padding:8px 0;border-bottom:1px solid #1e1e2e">
              <div>
                <span style="color:#fff;font-size:13px;font-weight:700">${v.aerolinea}${vueloStr && v.aerolinea !== vueloStr ? ` · ${vueloStr}` : ''}</span>
                ${horariosStr ? `<span style="color:#888;font-size:12px;display:block;margin-top:2px">🕐 ${horariosStr}${seatsStr ? `&nbsp;&nbsp;${seatsStr}` : ''}</span>` : (seatsStr ? `<span style="color:#2ecc71;font-size:12px;display:block;margin-top:2px">${seatsStr}</span>` : '')}
              </div>
              <div style="display:flex;align-items:center;gap:10px">
                <div style="text-align:right">
                  <span style="color:#fff;font-weight:700;font-size:13px">USD ${precioFinal}</span>
                  <span style="color:#666;font-size:10px;display:block">incl. carry on</span>
                </div>
                <a href="${waUrl}" style="background:#25D366;color:#fff;font-size:11px;font-weight:700;padding:5px 10px;border-radius:6px;text-decoration:none;white-space:nowrap">💬 Reservar</a>
              </div>
            </div>`;
        }).join('');
        html += `
          <div style="background:#13131f;border:1px solid #333;border-left:3px solid #2ecc71;border-radius:6px;padding:10px 14px;margin-bottom:8px">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
              <strong style="color:#fff;font-size:14px">TLV → ${e.destino.iata} (${e.destino.nombre})</strong>
              <span style="background:#2ecc71;color:#000;font-size:11px;font-weight:700;padding:2px 8px;border-radius:10px">${e.cantidad} tarifa${e.cantidad !== 1 ? 's' : ''} disponible${e.cantidad !== 1 ? 's' : ''}</span>
            </div>
            ${vuelosStr}
          </div>`;
      }
