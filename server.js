...
async function enviarEmailReserva(reserva, pasajeros, paxPrecios = null) {
  if (!resendClient) return;
  try {
    // Get contact info from user who created the reservation
    const usuarioContacto = await getContactoUsuario(reserva.usuario_id);
...
if (db) {
  (async () => {
    try {
      ...
      // Pre-populate contact data for existing users
      ...
      await db.query(`UPDATE usuarios SET email='Julietaz@luckytourviajes.com', telefono='+54 9 11 3295 5404' WHERE usuario='julieta' AND (email IS NULL OR email='')`);
      ...
      if (db) {
        try {
          ...
          try {
            ...
            if (resRow.rows.length) await enviarEmailReserva(resRow.rows[0], pasajeros, vueloInfo?.precioVentaDetalle || null);
...
app.post('/reservas/:id/pdf', async (req, res) => {
  ...
  try {
    ...
    const reqVendedorFinal = reqVendedor;
    const contactoDb = await getContactoUsuario(reserva.usuario_id);
    const contacto = contactoDb || CONTACTOS[reqVendedorFinal] || CONTACTOS.guido;
    const NAVY = '#1B3A5C';
...
app.post('/generar-cotizacion', async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      await resendClient.emails.send({
        from: cotizContacto ? `${cotizContacto.nombre} <${cotizContacto.mail}>` : 'Lucky Tour <guido@luckytourviajes.com>',
        to: EMAIL_FROM,
        subject: nombrePDF,
        html: '<p>&nbsp;</p>',
        attachments: [{
