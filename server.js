...
function procesarVuelosLleego(resp, paxCounts = { adultos: 1, ninos: 0, infantes: 0 }) {
  ...
  return solutions.flatMap((sol, solIdx) => {
    try {
      ...
      // Generar una opción por asociación (cada una es un itinerario distinto)
      const opcionesPorAssoc = assocsFull.length > 0 ? assocsFull.map((_, assocIdx) => assocIdx) : [0];

      return opcionesPorAssoc.map(assocIdx => {
      // Cache: guardar sol con solo la asociación correspondiente
      const solParaCache = { ...sol, data: { ...sol.data, associations: [assocsFull[assocIdx] || assocsFull[0]] } };
      const cacheKey = assocsFull.length > 1 ? `lleego_${sol.id}_a${assocIdx}` : `lleego_${sol.id}`;
