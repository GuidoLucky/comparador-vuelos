...
async function cronMonitorTLV() {
    ...
    await Promise.all(batch.map(async ({ fecha, dest }) => {
      try {
        ...
        if (token) {
          ...
          const payload = {
            itinerary: [{ departureAirportCode: 'TLV', arrivalAirportCode: dest.iata, departureDate: fecha }],
            adults: 1, children: 0, infants: 0,
            currencyCode: 'USD', solutionsPerPage: 50,
            stops: 0
          };
          const r = await fetch(`${API_BASE}/FlightAvailability/Search`, {
            method: 'POST', headers: getHeaders(token),
        ...
        if (llToken) {
          ...
          const llPayload = {
            ...
            segments: [{ departure: 'TLV', arrival: dest.iata, date: fecha }],
            max_stops: 0
          };
          const lr = await fetch('https://api-tr.lleego.com/api/v2/transport/search', {
            method: 'POST',
            headers: { 'Authorization': `Bearer ${llToken}`, 'x-api-key': LLEEGO_API_KEY, 'Content-Type': 'application/json', 'lang': 'es-ar' },
            body: JSON.stringify(llPayload)
          }).catch(() => null);
          if (lr?.ok) {
            const llData = await lr.json().catch(() => null);
            const searchToken = llData?.search_token || llData?.token;
            if (searchToken) {
              await new Promise(r => setTimeout(r, 8000)); // esperar 8s para que carguen los resultados
              const pr = await fetch(`https://api-tr.lleego.com/api/v2/transport/search/results?token=${searchToken}&locale=es-ar`, {
                headers: { 'Authorization': `Bearer ${llToken}`, 'x-api-key': LLEEGO_API_KEY }
              }).catch(() => null);
              if (pr?.ok) {
                const pData = await pr.json().catch(() => null);
                const sols = pData?.solutions || pData?.data?.solutions || [];
                console.log(`[TLV] GEA TLV→${dest.iata} ${fecha}: ${sols.length} soluciones totales`);
                const filtGEA = sols.filter(s => {
                  const provs = s.providers || [];
                  return provs.some(p => ['IZ','IS','LY'].includes((p.validating_carrier||'').toUpperCase()));
                });
                if (filtGEA.length) {
                  encontrados.push({
  ...
  if (encontrados.length) {
    console.log(`[TLV] ✅ ${encontrados.length} combinaciones con disponibilidad — enviando email`);
