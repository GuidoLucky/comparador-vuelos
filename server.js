...
// Helper: fetch GLAS con auto-reintento en 403/401
async function glasFetch(userId, url, options = {}) {
  let token = await getUserTucanoToken(userId);
  let resp = await fetch(url, { ...options, headers: getHeaders(token) });
  if (resp.status === 403 || resp.status === 401) {
    console.log(`[GLAS] Token inválido (${resp.status}), renovando...`);
    if (userTokenCaches[userId]) userTokenCaches[userId].tucano = { token: null, expiry: 0 };
    token = await getUserTucanoToken(userId);
    resp = await fetch(url, { ...options, headers: getHeaders(token) });
  }
  return resp;
}
...
app.post('/crear-reserva', async (req, res) => {
  ...
  try {
    ...
    let r = await fetch(`${API_BASE}/FlightReservation/CreateReservationRemake`, {
      method:'POST', headers: getHeaders(token), body: JSON.stringify(bookPayload)
    });
    // Si 403, limpiar caché y reintentar con token nuevo
    if (r.status === 403 || r.status === 401) {
      console.log(`[Reserva] Token inválido (${r.status}), renovando y reintentando...`);
      if (req.user?.userId && userTokenCaches[req.user.userId]) {
        userTokenCaches[req.user.userId].tucano = { token: null, expiry: 0 };
      }
      const newToken = await getUserTucanoToken(req.user?.userId);
      r = await fetch(`${API_BASE}/FlightReservation/CreateReservationRemake`, {
        method:'POST', headers: getHeaders(newToken), body: JSON.stringify(bookPayload)
      });
    }
...
app.post('/reservas/:id/verificar', async (req, res) => {
  ...
  try {
    ...
    let token = await getUserTucanoToken(reserva.usuario_id);
    let hdrs = getHeaders(token);

    // Endpoint real: POST FlightReservation/RetrieveReservation
    let resp = await fetch(`${API_BASE}/FlightReservation/RetrieveReservation`, {
      method: 'POST', headers: hdrs,
      body: JSON.stringify({ OrderId: reserva.order_id })
    });
    // Si 403/401, limpiar caché y reintentar
    if (resp.status === 403 || resp.status === 401) {
      console.log(`[Verificar] Token inválido (${resp.status}), renovando...`);
      if (userTokenCaches[reserva.usuario_id]) userTokenCaches[reserva.usuario_id].tucano = { token: null, expiry: 0 };
      token = await getUserTucanoToken(reserva.usuario_id);
      hdrs = getHeaders(token);
      resp = await fetch(`${API_BASE}/FlightReservation/RetrieveReservation`, {
        method: 'POST', headers: hdrs,
        body: JSON.stringify({ OrderId: reserva.order_id })
      });
    }
...
app.post('/reservas/:id/recotizar', async (req, res) => {
  ...
  try {
    ...
    const rrResp = await glasFetch(reserva.usuario_id, `${API_BASE}/FlightReservation/RetrieveReservation`, {
      method: 'POST', body: JSON.stringify({ OrderId: reserva.order_id })
    });
...
const TLV_FECHAS = ['2026-03-10', '2026-03-11', '2026-03-12', '2026-03-15', '2026-03-16', '2026-03-17', '2026-03-18', '2026-03-19'];
...
async function cronMonitorTLV() {
  // Apagar automáticamente después del 16/03/2026
  ...
  if (encontrados.length) {
    ...
    if (db) {
      for (const e of encontrados) {
        try {
          const v = e.vuelos[0] || {};
          const seatsStr = v.seats != null ? ` · ${v.seats} lugar${v.seats !== 1 ? 'es' : ''}` : (e.cantidad ? ` · ${e.cantidad} lugar/es` : '');
          const fechaFmt = new Date(e.fecha + 'T12:00:00').toLocaleDateString('es-AR', { weekday: 'short', day: 'numeric', month: 'short' });
          const horario = (v.salida && v.llegada) ? ` ${v.salida}→${v.llegada}` : '';
          const precio = v.precio ? ` — USD ${Math.ceil(v.precio + 40)}` : '';
          const mensaje = `✈ TLV → ${e.destino.iata} (${e.destino.nombre}) ${fechaFmt} — ${v.aerolinea || '?'}${v.vuelo ? ' ' + v.vuelo : ''}${horario}${seatsStr}${precio}`;
          const existe = await db.query(
            `SELECT id FROM notificaciones WHERE tipo='tlv_disponibilidad' AND mensaje=$1 AND leida=false AND created_at > NOW() - INTERVAL '30 minutes'`,
            [mensaje]
          );
          if (existe.rows.length) continue;
          await db.query(
            `INSERT INTO notificaciones (tipo, mensaje, detalle) VALUES ($1, $2, $3)`,
            ['tlv_disponibilidad', mensaje, JSON.stringify(e)]
          );
        } catch(e2) { console.log('[TLV] Error insertando notif:', e2.message); }
      }
    }
  } else {
