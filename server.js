...
async function enviarEmailReserva(reserva, pasajeros) {
  ...
  try {
    ...
    function buildPrecioVentaHtml(paxArray, esCliente = false) {
      ...
      const tipoLabel = { adulto: 'adulto', menor: 'menor', 'bebé': 'bebé', bebe: 'bebé' };
      const grupos = {};
      for (const p of paxArray) {
        const t = (p.tipo || 'adulto').toLowerCase().replace('é','e');
        if (!grupos[t]) grupos[t] = { precio: p.neto || 0, cantidad: 0, tipo: tipoLabel[t] || t };
        grupos[t].cantidad += (p.cantidad || 1);
      }
      let rows = '';
      for (const g of Object.values(grupos)) {
        if (!g.precio) continue;
        const label = g.cantidad > 1 ? `USD ${g.precio.toLocaleString('es-AR')} cada ${g.tipo}` : `USD ${g.precio.toLocaleString('es-AR')} ${g.tipo}`;
        const subtotal = g.precio * g.cantidad;
        rows += `<tr>
          <td style="padding:8px 12px;color:#444;">${label}</td>
          <td style="padding:8px 12px;text-align:right;color:#222;font-weight:${g.cantidad > 1 ? 'normal' : 'bold'};">
            ${g.cantidad > 1 ? `× ${g.cantidad} = <strong>USD ${subtotal.toLocaleString('es-AR')}</strong>` : ''}
          </td>
        </tr>`;
      }
      if (!rows) return '';
      const total = reserva.precio_venta_usd;
      return `
        <h3 style="color:#333;margin:20px 0 10px;">${esCliente ? '💰 Precio' : '💰 Precio de venta'}</h3>
        <table style="width:100%;border-collapse:collapse;background:#f9f9f9;border-radius:6px;overflow:hidden;">
          ${rows}
          ${total ? `<tr style="background:#1a1a2e;color:white;">
            <td style="padding:10px 12px;font-weight:bold;">Total</td>
            <td style="padding:10px 12px;text-align:right;font-weight:bold;">USD ${Number(total).toLocaleString('es-AR')}</td>
          </tr>` : ''}
        </table>`;
    }
    ...
    const precioVentaHtml = buildPrecioVentaHtml(pasajeros, false);
    ...
    console.log(`[Email] contacto_json:`, JSON.stringify(contacto), `| clienteEmail: ${clienteEmail} | emisionToken: ${emisionToken}`);
    ...
    if (clienteEmail && emisionToken) {
      const precioClienteHtml = buildPrecioVentaHtml(pasajeros, true);
      const htmlCliente = `
