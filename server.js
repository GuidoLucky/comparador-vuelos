  ...
  try {
    ...
    const r = await db.query(`
      SELECT id, pnr, order_id, estado, gds, notas, fecha_salida, aerolinea, usuario_id,
             origen, destino, itinerario_json, cantidad_adultos, cantidad_ninos, cantidad_infantes,
      ...
      try {
        ...
        try {
          ...
          if (itin?.length) {
            ...
            // Si el último tramo ya llegó hace más de 2 días, saltear
            if (ultimaFecha && ultimaFecha < Date.now() - 2 * 24 * 60 * 60 * 1000) {
              console.log(`[Cron-Verify] ${reserva.pnr}: viaje finalizado, marcando cron_finalizado`);
        ...
        if (estadoCambio || hayCambios) {
          try {
            ...
            if (necesitaAlternativas) {
              ...
              alternativas = await buscarAlternativasCron(reserva, reserva.usuario_id);
...
async function buscarAlternativasCron(reserva, userId) {
  ...
  try {
    ...
    const infantes   = reserva.cantidad_infantes || 0;
