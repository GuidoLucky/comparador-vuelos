...
async function cronMonitorTLV() {
  ...
  console.log('[TLV] Iniciando búsqueda...');
        ...
        if (token) {
          ...
          if (r?.ok) {
            ...
            const filtrados = vuelos.filter(v => {
              if (v.escalas !== 0) return false; // doble check directo
  ...
  if (encontrados.length) {
    ...
    console.log('[TLV] Sin disponibilidad por ahora');
  }
}
...
app.post('/tlv/monitor', requireAuth, (req, res) => {
...
app.get('/tlv/monitor', requireAuth, (req, res) => res.json({ activo: tlvMonitorActivo }));
app.post('/tlv/test', requireAuth, async (req, res) => {
