      ...
      try {
        ...
        if (llToken) {
          ...
          if (lr?.ok) {
            ...
            const filtGEA = sols.filter(s => {
              const carrier = (s.providers?.[0]?.validating_carrier || s.validating_carrier || '').toUpperCase();
              return TLV_AEROLINEAS.includes(carrier);
            });
            ...
            if (filtGEA.length) {
              encontrados.push({
                ...
                vuelos: filtGEA.slice(0, 3).map(s => ({
                  precio: s.total_price?.total || 0,
                  aerolinea: s.providers?.[0]?.validating_carrier || '?'
                }))
              });
            }
          } else {
