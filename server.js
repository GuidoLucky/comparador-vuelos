...
async function getUserTucanoToken(userId) {
  ...
  if (!cred) throw new Error('No tienes acceso a Tucano. Pedile al administrador que configure tus credenciales.');
...
async function getUserLleegoToken(userId) {
  ...
  if (!cred) throw new Error('No tienes acceso a GEA. Pedile al administrador que configure tus credenciales.');
  
  const extra = typeof cred.cred_extra === 'string' ? JSON.parse(cred.cred_extra) : (cred.cred_extra || {});
  const agent = extra.agent || 'GFinkelstein';
  
  const geaAuthUrl = `https://middle.lleego.com/api/user/auto-login?e=${encodeURIComponent(cred.cred_user)}&w=${encodeURIComponent(cred.cred_pass)}&idAgente=${encodeURIComponent(agent)}`;
  const authRes = await fetch(geaAuthUrl, {
    headers: { 'Accept': 'application/json', 'Content-Type': 'application/json', 'env': 'geaargentina', 'lang': 'es-ar' }
  });
  if (!authRes.ok) throw new Error('Credenciales GEA incorrectas. Verificá tus datos de acceso.');
  const authData = await authRes.json();
  const token = authData.token;
  if (!token) throw new Error('No se pudo obtener token GEA. Verificá tus credenciales.');
...
async function getContactoUsuario(userId) {
  if (!db || !userId) return null;
  try {
    const r = await db.query('SELECT nombre, email, telefono FROM usuarios WHERE id=$1', [userId]);
    const u = r.rows[0];
    if (!u) return null;
    return { nombre: u.nombre, mail: u.email || '', tel: u.telefono || '' };
  } catch(e) { return null; }
}

async function enviarEmailEmision(reserva) {
  try {
    const pnr = reserva.pnr || '';
    const notas = reserva.notas || '';
    const quotationId = reserva.quotation_id || '';
    const isGEA = quotationId.startsWith('lleego_') || notas.includes('GEA');

    let destinatario;
    if (isGEA) {
      destinatario = 'aereos@travelgea.la';
    } else {
      // Horario Argentina GMT-3
      const now = new Date();
      const offsetMs = -3 * 60 * 60 * 1000;
      const ar = new Date(now.getTime() + now.getTimezoneOffset() * 60000 + offsetMs);
      const diaSemana = ar.getDay(); // 0=Dom, 6=Sab
      const horas = ar.getHours() + ar.getMinutes() / 60;
      const esHorarioLaboral = diaSemana >= 1 && diaSemana <= 5 && horas >= 9.5 && horas < 18;
      destinatario = esHorarioLaboral ? 'atencion@tucanotours.com.ar' : 'guardia@tucanotours.com.ar';
    }

    const emisContacto = await getContactoUsuario(reserva.usuario_id);
    const emisFrom = emisContacto ? `${emisContacto.nombre} <${emisContacto.mail}>` : 'Lucky Tour <guido@luckytourviajes.com>';
    await resendClient.emails.send({
      from: emisFrom,
      to: destinatario,
      subject: `PDF - ${pnr}`,
      html: `<p>Hola! Por favor enviar como pdf sin precio los tickets de la reserva ${pnr}</p>`
    });

    await db.query('UPDATE reservas SET emision_mail_enviado=true WHERE id=$1', [reserva.id]);
    console.log(`[EmisionMail] Email enviado a ${destinatario} para ${pnr}`);
  } catch(e) {
    console.error('[EmisionMail] Error:', e.message);
  }
}

async function enviarEmailReserva(reserva, pasajeros, paxPrecios = null, userId = null) {
  if (!resendClient) return;
  try {
    const usuarioContacto = await getContactoUsuario(userId || reserva.usuario_id);
    const fromEmail = usuarioContacto?.mail || 'guido@luckytourviajes.com';
    const fromName = usuarioContacto?.nombre || 'Lucky Tour';
    const fromField = `${fromName} <${fromEmail}>`;
    const paxNombres = pasajeros.map(p => `${p.apellido}, ${p.nombre} (${p.tipo || 'ADT'})`).join('<br>');

    // Generate emision token if not exists
    let emisionToken = reserva.emision_token;
    if (!emisionToken && db && reserva.pnr) {
      emisionToken = crypto.randomBytes(24).toString('hex');
      await db.query('UPDATE reservas SET emision_token=$1 WHERE pnr=$2', [emisionToken, reserva.pnr]);
    }
    const emisionUrl = `https://comparador-vuelos-production.up.railway.app/solicitar-emision/${emisionToken}`;

    // Build price breakdown HTML (precio de venta por tipo)
    function buildPrecioVentaHtml(paxArray, esCliente = false) {
      if (!paxArray || !paxArray.length) return '';
      // Soporta precioVentaDetalle {tipo, cantidad, precioUnitario} y pasajeros {tipo, cantidad, neto}
      const tipoDisplay = { adulto: 'adulto', menor: 'menor', bebe: 'bebé', adt: 'adulto', chd: 'menor', cnn: 'menor', inf: 'bebé' };
      const grupos = {};
      for (const p of paxArray) {
        const t = (p.tipo || 'adulto').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');
        const precio = Number(p.precioUnitario || p.neto || p.precioVenta || 0);
        if (!grupos[t]) grupos[t] = { precio, cantidad: 0, display: tipoDisplay[t] || p.tipo || t };
        grupos[t].cantidad += Number(p.cantidad || 1);
      }
      let rows = '';
      for (const g of Object.values(grupos)) {
        if (!g.precio) continue;
        const label = g.cantidad > 1
          ? `USD ${g.precio.toLocaleString('es-AR')} cada ${g.display}`
          : `USD ${g.precio.toLocaleString('es-AR')} ${g.display}`;
        rows += `<tr><td style="padding:10px 14px;font-size:15px;"><strong>${label}</strong></td></tr>`;
      }
      if (!rows) return '';
      return `<h3 style="color:#333;margin:20px 0 10px;">${esCliente ? '💰 Precio' : '💰 Precio de venta'}</h3>
        <table style="width:100%;border-collapse:collapse;background:#f9f9f9;border-radius:6px;overflow:hidden;">${rows}</table>`;
    }
    ...
    const precioVentaHtml = buildPrecioVentaHtml(paxPrecios || pasajeros, false);
    ...
    try {
      const pdfBuf = await generarPDFReservaBuffer(reserva.id, userId || reserva.usuario_id);
      pdfAttachment = { filename: `Reserva_${reserva.pnr}.pdf`, content: pdfBuf.toString('base64') };
    } catch(pdfErr) {
      console.error('[Email] No se pudo generar PDF adjunto:', pdfErr.message);
    }
    ...
    console.log(`[Email] Reserva ${reserva.pnr} enviada a ${fromEmail}${pdfAttachment ? ' (con PDF)' : ''}`);

    // ─── Email al cliente ───
    const contacto = (typeof reserva.contacto_json === 'string' ? JSON.parse(reserva.contacto_json || '{}') : (reserva.contacto_json || {}));
    const clienteEmail = contacto.email || '';
    const clienteNombre = contacto.nombre || contacto.name ||
      (pasajeros[0] ? `${pasajeros[0].nombre} ${pasajeros[0].apellido}` : '');

    if (clienteEmail && emisionToken) {
      const precioClienteHtml = buildPrecioVentaHtml(paxPrecios || pasajeros, true);
      const htmlCliente = `
      <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;background:#f0f4f8;padding:20px;">
        <div style="background:#1a1a2e;color:white;padding:28px 24px;border-radius:8px 8px 0 0;text-align:center;">
          <h1 style="margin:0;font-size:26px;">✈️ Lucky Tour</h1>
          <p style="margin:8px 0 0;opacity:0.8;font-size:15px;">Tu reserva fue confirmada</p>
        </div>
        <div style="background:white;padding:28px 24px;border-radius:0 0 8px 8px;box-shadow:0 2px 8px rgba(0,0,0,0.1);">

          <p style="font-size:16px;color:#333;margin-bottom:20px;">Hola <strong>${clienteNombre}</strong>, tu reserva está confirmada y pendiente de emisión.</p>

          <div style="background:#f0f7ff;border-left:4px solid #c9a84c;padding:14px;margin-bottom:20px;border-radius:4px;">
            <div style="font-size:13px;color:#888;margin-bottom:4px;">Código de reserva</div>
            <div style="font-size:28px;font-weight:bold;color:#c9a84c;letter-spacing:3px;">${reserva.pnr}</div>
            <div style="margin-top:6px;color:#444;">Aerolínea: <strong>${aerolinea}</strong></div>
          </div>

          <h3 style="color:#333;margin:20px 0 10px;">🛫 Itinerario</h3>
          ${segmentosHtml ? `
          <table style="width:100%;border-collapse:collapse;margin-bottom:20px;">
            <thead>
              <tr style="background:#f5f5f5;">
                <th style="padding:8px;text-align:left;font-size:13px;color:#666;">Ruta</th>
                <th style="padding:8px;text-align:left;font-size:13px;color:#666;">Salida</th>
                <th style="padding:8px;text-align:left;font-size:13px;color:#666;">Llegada</th>
              </tr>
            </thead>
            <tbody>${segmentosHtml}</tbody>
          </table>` : ''}
          ...
          ${reserva.time_limit ? `
          <div style="background:#fff8e1;border:1px solid #ffc107;padding:12px;border-radius:4px;margin:20px 0;text-align:center;">
            ⏰ <strong>Límite para emitir: ${fmtFecha(reserva.time_limit)}</strong>
          </div>` : ''}



          <p style="color:#ccc;font-size:11px;margin-top:24px;text-align:center;border-top:1px solid #eee;padding-top:16px;">
            Lucky Tour Viajes · guido@luckytourviajes.com · Este email fue generado automáticamente
          </p>
        </div>
      </div>`;
      ...
      await resendClient.emails.send({
        from: fromField,
        to: clienteEmail,
        subject: `✈️ Tu reserva ${reserva.pnr} - ${reserva.origen || ''} → ${reserva.destino || ''}`,
        html: htmlCliente
      });
      console.log(`[Email] Email cliente ${reserva.pnr} enviado a ${clienteEmail}`);
    }
...
if (db) {
  (async () => {
    try {
      ...
      await db.query(`ALTER TABLE reservas ADD COLUMN IF NOT EXISTS emision_token TEXT`);
      await db.query(`ALTER TABLE reservas ADD COLUMN IF NOT EXISTS emision_solicitada_at TIMESTAMPTZ`);
      await db.query(`ALTER TABLE reservas ADD COLUMN IF NOT EXISTS precio_venta_detalle JSONB`);
      await db.query(`ALTER TABLE reservas ADD COLUMN IF NOT EXISTS emision_mail_enviado BOOLEAN DEFAULT false`);
      await db.query(`ALTER TABLE usuarios ADD COLUMN IF NOT EXISTS email TEXT`);
      await db.query(`ALTER TABLE usuarios ADD COLUMN IF NOT EXISTS telefono TEXT`);
      await db.query(`UPDATE usuarios SET email='Guido@luckytourviajes.com', telefono='+54 9 11 6846 3892' WHERE usuario='guido' AND (email IS NULL OR email='')`);
      await db.query(`UPDATE usuarios SET email='Ventas@luckytourviajes.com', telefono='+54 9 11 6847 0986' WHERE (usuario='ruthytuch' OR usuario='ruth' OR nombre ILIKE '%Ruthy%' OR nombre ILIKE '%Ruth%') AND (email IS NULL OR email='')`);
      await db.query(`UPDATE usuarios SET email='Julietaz@luckytourviajes.com', telefono='+54 9 11 3295 5404' WHERE (usuario='julieta' OR nombre ILIKE '%Julieta%') AND (email IS NULL OR email='')`);
      ...
      await db.query(`CREATE TABLE IF NOT EXISTS tlv_suscriptores (
        id SERIAL PRIMARY KEY,
        nombre TEXT NOT NULL,
      ...
      // Insertar guido@ como suscriptor por defecto si no existe
      await db.query(`INSERT INTO tlv_suscriptores (nombre, email) VALUES ('Guido', 'guido@luckytourviajes.com') ON CONFLICT (email) DO NOTHING`);
      console.log('[DB] TLV suscriptores OK');
    ...
    for (const group of itinGroups) {
      ...
      for (const itin of itineraries) {
        try {
          ...
          // Baggage per leg — Sabre baggageInformation has segmentIndex
          let equipajePorTramo = legs.map((leg, i) => ({
            legIndex: i, origen: leg.origen, destino: leg.destino,
          ...
          try {
            const pi = paxInfoList[0]?.passengerInfo || {};
            const bagInfos = pi.baggageInformation || [];
            // Map segment index → leg index using legs[].segmentos count
            const segToLeg = [];
            legs.forEach((leg, li) => { (leg.segmentos||[leg]).forEach(() => segToLeg.push(li)); });
            // Build per-leg checked info
            const checkedPerLeg = {};
            for (const bag of bagInfos) {
              const allowance = baggageMap[bag.allowance?.ref];
              if (!allowance) continue;
              const segIdx = bag.segmentIndex ?? 0;
              const legIdx = segToLeg[segIdx] ?? 0;
              if (!checkedPerLeg[legIdx]) {
                const pieces = allowance.pieceCount ?? 0;
                const weight = allowance.weight;
                checkedPerLeg[legIdx] = {
                  label: pieces > 0 ? (weight ? `${pieces}x ${weight}${allowance.unit||'KG'}` : `${pieces}x 23KG`) : 'No incluida',
                  incluido: pieces > 0 || !!weight
                };
              }
            }
            equipajePorTramo = legs.map((leg, i) => ({
              legIndex: i, origen: leg.origen, destino: leg.destino,
              carryOn: { label: 'No informado', incluido: false },
              checked: checkedPerLeg[i] || { label: 'No incluida', incluido: false }
            }));
            equipaje = {
              handOn: { label: 'Incluida', incluido: true },
              carryOn: { label: 'No informado', incluido: false },
              checked: { label: equipajePorTramo[0]?.checked.label || 'No informado', incluido: equipajePorTramo.every(t => t.checked.incluido) }
            };
          } catch(e) {}
...
function procesarVuelosLleego(resp, paxCounts = { adultos: 1, ninos: 0, infantes: 0 }) {
  ...
  return solutions.flatMap((sol, solIdx) => {
    try {
      const searchToken = resp.token || '';
      const assocsFull = sol.data?.associations || [];

      // Cada asociación es un TRAMO del itinerario (no itinerarios separados)
      // Dentro de cada asociación puede haber múltiples journey_references (opciones de vuelo para ese tramo)
      // Hay que generar el producto cartesiano: una opción por cada combinación posible de journeys entre tramos
      
      // Para cada asociación (tramo), extraer sus journey options
      const opcionesPorTramo = assocsFull.map(assoc => assoc.journey_references || []);
      ...
      const combinaciones = cartesian(opcionesPorTramo);

      // Si no hay combinaciones, usar una vacía
      const combsToProcess = combinaciones.length > 0 ? combinaciones : [[]];
      
      // Log primera solución
      if (solIdx === 0) {
        console.log(`[Lleego] Sol[0] assocs (tramos): ${assocsFull.length}, combinaciones: ${combinaciones.length}`);
        assocsFull.forEach((a, i) => console.log(`[Lleego] Sol[0] tramo[${i}] journey options:`, a.journey_references));
      }

      return combsToProcess.map((combJourneys, combIdx) => {
        // Construir asociaciones con un journey por tramo para esta combinación
        const assocsParaComb = assocsFull.map((assoc, i) => ({
          ...assoc,
          journey_references: combJourneys[i] ? [combJourneys[i]] : (assoc.journey_references || []).slice(0, 1)
        }));
        const solParaCache = { ...sol, data: { ...sol.data, associations: assocsParaComb } };
        const cacheKey = combsToProcess.length > 1 ? `lleego_${sol.id}_c${combIdx}` : `lleego_${sol.id}`;
        lleegoSolutionsCache.set(cacheKey, { sol: solParaCache, segments, journeys, fares, companies, providers, ports, searchToken, paxCounts });
        if (combIdx === 0) lleegoSolutionsCache.set(`lleego_${sol.id}`, { sol: solParaCache, segments, journeys, fares, companies, providers, ports, searchToken, paxCounts });
      
      // Log first solution structure for debugging
      if (solIdx === 0 && combIdx === 0) {
        ...
        if (a0) console.log('[Lleego] First assoc keys:', Object.keys(a0));
      }
      // Get associations (ida y vuelta) — usar la combinación actual
      const assocs = assocsParaComb;
      const itinerario = [];
      let maxEscalas = 0;
      ...
      for (const assoc of assocs) {
        ...
        for (const jRef of journeyRefs) {
          ...
          // No break - cada asociación tiene exactamente un journey en la combinación
        }
      }
      ...
      // Baggage per leg (association = tramo ida/vuelta)
      const equipajePorTramo = assocs.map((assoc, i) => {
        const segRefs = Object.values(assoc.segment_references || {});
        const seg = segRefs[0] || {};
        const baggage = seg.baggage || {};
        const cabinBag = seg.cabin_baggage || {};
        const checkedQty = baggage.quantity || 0;
        const checkedIncluido = checkedQty > 0;
        const leg = itinerario[i] || {};
        return {
          legIndex: i,
          origen: leg.origen || '',
          destino: leg.destino || '',
          carryOn: { label: cabinBag.included ? 'Incluido' : 'No incluido', incluido: !!cabinBag.included },
          checked: { label: checkedIncluido ? `${checkedQty}x 23KG` : 'No incluida', incluido: checkedIncluido }
        };
      });

      // Fare basis from first assoc
      ...
      // Global equipaje conservador
      const equipaje = {
        handOn: { label: 'Incluida', incluido: true },
        carryOn: { label: equipajePorTramo[0]?.carryOn.label || 'No informado', incluido: equipajePorTramo.every(t => t.carryOn.incluido) },
        checked: { label: equipajePorTramo[0]?.checked.label || 'No informado', incluido: equipajePorTramo.every(t => t.checked.incluido) }
      };
    ...
    try {
      ...
      if (!searchRes.ok) {
        if (searchRes.status === 403 || searchRes.status === 401) {
          ...
          console.log(`[GLAS] Token inválido (${searchRes.status}), reintentando...`);
          const newToken = await getToken();
          const retryRes = await fetch(endpoint, {
            method:'POST', headers: getHeaders(newToken), body: JSON.stringify(payload)
          });
          if (!retryRes.ok) throw new Error(`API error: ${retryRes.status}`);
          const retryData = await retryRes.json();
          console.log(`[GLAS] Reintento OK: ${retryData.minifiedQuotations?.length || 0} resultados`);
          const vuelosRetry = procesarVuelos(retryData, stopsFilter);
          vuelosRetry.forEach(v => { v.fuente = 'Tucano'; v.gds = v.source || ''; });
          return { vuelos: vuelosRetry, searchId: retryData.searchId || retryData.SearchId };
        }
      ...
      if (err.message.includes('401') || err.message.includes('403')) tokenCache = { token:null, expiry:0 };
    ...
    .map(q => {
      ...
      function parseBagLeg(bagLeg) {
        ...
        const checkedLabel = checkedItem
          ...
          : 'No incluida';
        return { carryOn: { label: carryOnLabel, incluido: carryOnIncluido }, checked: { label: checkedLabel, incluido: checkedIncluido } };
      }

      const legsWithBag = q.legsWithBaggageAllowance || [];
      const equipajePorTramo = itinerario.map((leg, i) => {
        const bagEntry = legsWithBag[i]?.baggageAllowance || legsWithBag[0]?.baggageAllowance;
        return { legIndex: i, origen: leg.origen, destino: leg.destino, ...parseBagLeg(bagEntry) };
      });
      // Equipaje global = conservador (si algún tramo no incluye, el global tampoco)
      const equipaje = {
        handOn: { label: 'Incluida', incluido: true },
        carryOn: { label: equipajePorTramo[0]?.carryOn.label || 'No informado', incluido: equipajePorTramo.every(t => t.carryOn.incluido) },
        checked: { label: equipajePorTramo[0]?.checked.label || 'No informado', incluido: equipajePorTramo.every(t => t.checked.incluido) }
...
app.post('/get-fare-breakdown', async (req, res) => {
  ...
  try {
    ...
    const breakdown = rates.map(rate => {
      ...
      const comOver = rate.discountAmount || 0;
      return {
        tipo,
        codigo: code,
...
// ─── SOLICITAR EMISIÓN ───
app.get('/solicitar-emision/:token', async (req, res) => {
  const { token } = req.params;
  try {
    const row = await db.query('SELECT * FROM reservas WHERE emision_token=$1 LIMIT 1', [token]);
    if (!row.rows.length) {
      return res.status(404).send(`<html><body style="font-family:Arial;text-align:center;padding:60px;background:#f5f5f5;">
        <h2 style="color:#cc0000;">Enlace no válido</h2>
        <p>Este enlace no existe o ya fue utilizado.</p>
      </body></html>`);
    }
    const reserva = row.rows[0];

    // Mark as requested
    if (!reserva.emision_solicitada_at) {
      await db.query('UPDATE reservas SET emision_solicitada_at=NOW() WHERE emision_token=$1', [token]);

      // Send alert email to vendor
      if (resendClient) {
        const contacto = (typeof reserva.contacto_json === 'string' ? JSON.parse(reserva.contacto_json || '{}') : (reserva.contacto_json || {}));
        const clienteNombre = contacto.nombre || contacto.name || (reserva.pasajeros_json?.[0] ? `${reserva.pasajeros_json[0].nombre} ${reserva.pasajeros_json[0].apellido}` : 'el pasajero');
        const clienteEmail = contacto.email || '';
        const clienteTel = contacto.telefono || contacto.phone || '';

        await resendClient.emails.send({
          from: 'Lucky Tour <guido@luckytourviajes.com>',
          to: EMAIL_FROM,
          subject: `🚨 Solicitud de emisión: ${reserva.pnr} - ${reserva.origen || ''} → ${reserva.destino || ''}`,
          html: `
            <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;background:#f9f9f9;padding:20px;">
              <div style="background:#1a1a2e;color:white;padding:20px;border-radius:8px 8px 0 0;text-align:center;">
                <h1 style="margin:0;font-size:24px;">🚨 Solicitud de Emisión</h1>
              </div>
              <div style="background:white;padding:24px;border-radius:0 0 8px 8px;box-shadow:0 2px 8px rgba(0,0,0,0.1);">
                <div style="background:#fff3cd;border-left:4px solid #ffc107;padding:16px;border-radius:4px;margin-bottom:20px;">
                  <p style="margin:0;font-size:18px;"><strong>${clienteNombre}</strong> solicitó la emisión de su reserva.</p>
                </div>
                <table style="width:100%;border-collapse:collapse;">
                  <tr><td style="padding:8px;color:#666;width:140px;">PNR</td><td style="padding:8px;font-weight:bold;font-size:18px;color:#c9a84c;">${reserva.pnr}</td></tr>
                  <tr style="background:#f9f9f9;"><td style="padding:8px;color:#666;">Ruta</td><td style="padding:8px;">${reserva.origen || ''} → ${reserva.destino || ''}</td></tr>
                  <tr><td style="padding:8px;color:#666;">Aerolínea</td><td style="padding:8px;">${reserva.aerolinea || '-'}</td></tr>
                  ${clienteEmail ? `<tr style="background:#f9f9f9;"><td style="padding:8px;color:#666;">Email cliente</td><td style="padding:8px;">${clienteEmail}</td></tr>` : ''}
                  ${clienteTel ? `<tr><td style="padding:8px;color:#666;">Teléfono</td><td style="padding:8px;">${clienteTel}</td></tr>` : ''}
                </table>
                <div style="text-align:center;margin-top:24px;">
                  <a href="https://comparador-vuelos-production.up.railway.app" style="background:#c9a84c;color:white;padding:12px 28px;border-radius:6px;text-decoration:none;font-weight:bold;font-size:15px;">Ir al sistema →</a>
                </div>
              </div>
            </div>`
        });
        console.log(`[Email] Solicitud emisión ${reserva.pnr} notificada al vendedor`);
      }
    }

    // If already requested, show different message
    const yasolicitado = !!reserva.emision_solicitada_at;

    // Show confirmation page to client
    res.send(`<!DOCTYPE html>
<html lang="es">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Solicitud enviada — Lucky Tour</title>
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .header { background: #1a1a2e; color: white; padding: 32px 24px; text-align: center; }
    .header h1 { font-size: 22px; margin-bottom: 6px; }
    .header p { opacity: 0.75; font-size: 14px; }
    .body { padding: 32px 24px; text-align: center; }
    .check { font-size: 64px; margin-bottom: 16px; }
    .body h2 { color: #1a1a2e; font-size: 20px; margin-bottom: 12px; }
    .body p { color: #666; line-height: 1.6; font-size: 15px; }
    .pnr { display: inline-block; background: #f0f4f8; border-radius: 6px; padding: 8px 20px; margin: 16px 0; font-size: 22px; font-weight: bold; color: #c9a84c; letter-spacing: 2px; }
...
<body>
  <div class="card">
    ...
    <div class="body">
      <div class="check">${yasolicitado ? '⏳' : '✅'}</div>
      <h2>${yasolicitado ? 'Ya enviaste tu solicitud' : '¡Solicitud enviada!'}</h2>
      <p>${yasolicitado ? 'Ya recibimos tu pedido anteriormente.' : 'Tu solicitud de emisión fue recibida.'}</p>
      <div class="pnr">${reserva.pnr}</div>
      <p>${yasolicitado ? 'Estamos procesando la emisión de tu pasaje. Te contactamos a la brevedad.' : 'Nos estamos comunicando con vos a la brevedad para confirmar la emisión de tu pasaje.'}</p>
    </div>
    <div class="footer">
      <p>Lucky Tour Viajes · guido@luckytourviajes.com</p>
    </div>
  </div>
</body>
</html>`);
  } catch(e) {
    console.error('[SolicitarEmision] Error:', e.message);
...
app.post('/crear-reserva', async (req, res) => {
        ...
        try {
          await db.query(`INSERT INTO reservas (
            ...
            [pnr, pnr, String(quotationId),
             ...
             'USD',
             vueloInfo?.precioVentaDetalle ? JSON.stringify(vueloInfo.precioVentaDetalle) : null]);
          ...
          try {
            ...
            if (resRow.rows.length) await enviarEmailReserva(resRow.rows[0], pasajeros, vueloInfo?.precioVentaDetalle || null, req.user?.userId);
  ...
  if (String(quotationId).startsWith('lleego_')) {
    try {
      ...
      const llToken = await getUserLleegoToken(req.user?.userId);
      ...
      if (db) {
        try {
          ...
          const segmentosDetail = (vueloInfo?.itinerario?.flatMap(leg => leg.segmentos || []) || [])
            .map(s => ({
              vuelo: (s.vuelo || s.flightNumber || '').replace(/\s/g, ''), // sin espacios
              origen: s.origen || s.departureAirportCode || '',
              destino: s.destino || s.arrivalAirportCode || '',
              salida: s.salida || s.departureDateTime || '',
              llegada: s.llegada || s.arrivalDateTime || '',
              aerolinea: s.aerolinea || (s.vuelo || '').substring(0, 2)
            }))
  ...
  // ─── GLAS / Tucano booking ───
  try {
    const token = await getUserTucanoToken(req.user?.userId);
      ...
      try {
        ...
        const resIns = await db.query(`INSERT INTO reservas (
          ...
          [data.recordLocator, data.orderId, data.orderNumber, data.source,
           ...
           JSON.stringify([]), // segmentos_json: se llena con retrieve real justo abajo
        ...
        // ── Retrieve para guardar segmentos reales con números de vuelo ──
        try {
          const retrieveRes = await fetch(`${API_BASE}/FlightReservation/RetrieveReservation`, {
            method: 'POST', headers: getHeaders(token),
            body: JSON.stringify({ OrderId: data.orderId })
          });
          if (retrieveRes.ok) {
            const retrieveData = await retrieveRes.json();
            const flightsInfo = retrieveData.flightsInformation || [];
            if (flightsInfo.length) {
              // Normalizar segmentos con número de vuelo, origen, destino, salida, llegada
              const segmentosReales = flightsInfo.map(f => ({
                vuelo: f.flightNumber || '',
                origen: f.departureAirportCode || '',
                destino: f.arrivalAirportCode || '',
                salida: f.departureDateTime || f.departureDate || '',
                llegada: f.arrivalDateTime || f.arrivalDate || '',
                aerolinea: (f.flightNumber || '').substring(0, 2)
              }));
              await db.query('UPDATE reservas SET segmentos_json=$1 WHERE pnr=$2',
                [JSON.stringify(segmentosReales), data.recordLocator]);
              console.log(`[DB] Segmentos reales guardados para ${data.recordLocator}: ${segmentosReales.map(s=>s.vuelo).join(', ')}`);
            }
          }
        } catch(retrieveErr) {
          console.warn('[DB] No se pudo guardar segmentos reales:', retrieveErr.message);
        }
...
app.post('/reservas/:id/verificar', async (req, res) => {
  ...
  try {
    ...
    if (isGEA) {
      try {
        const llToken = await getUserLleegoToken(reserva.usuario_id);
    ...
    const token = await getUserTucanoToken(reserva.usuario_id);
    ...
    if (isSabre) {
      try {
        ...
        if (ticketRes.ok) {
          ...
          if (!reserva.emision_mail_enviado) await enviarEmailEmision(reserva);
...
// ─── FUNCIÓN REUTILIZABLE: genera PDF buffer de una reserva ───
async function generarPDFReservaBuffer(reservaId, usuarioIdOverride = null) {
    const r = await db.query(`
      SELECT r.*, 
    ...
    const reqVendedor = null;
    let preciosVenta = [];
    const tipoLabels = { ADT: 'adulto', CHD: 'menor', CNN: 'menor', INF: 'infante' };

    // Prioridad 1: usar precio_venta_detalle guardado en DB (precio exacto del momento de la reserva)
    const pvd = reserva.precio_venta_detalle;
    const pvdArray = pvd ? (typeof pvd === 'string' ? JSON.parse(pvd) : pvd) : null;
    if (pvdArray && pvdArray.length) {
      preciosVenta = pvdArray.map(p => ({
        tipo: (p.tipo || 'adulto').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, ''),
        cantidad: Number(p.cantidad || 1),
        neto: Number(p.precioUnitario || p.neto || p.precioVenta || 0),
        tipo_tarifa: 'CUSTOM',
        comision_over: 0
      }));
      console.log('[PDF] Usando precio_venta_detalle DB:', JSON.stringify(preciosVenta));
    ...
    const contactoDb = await getContactoUsuario(reserva.usuario_id);
    const contacto = contactoDb || CONTACTOS[reqVendedor] || CONTACTOS.guido;
    const NAVY = '#1B3A5C';
    ...
    const pdfTitulo = reserva.estado === 'EMITIDA' ? 'Reserva emitida' : 'Confirmación de Reserva';
    ...
    // ── CONDICIONES (solo si NO está emitida) ──
    if (!esEmitida) {
...
app.post('/reservas/:id/pdf', async (req, res) => {
  if (!db) return res.json({ ok: false, error: 'Sin DB' });
  try {
    const pdfBuffer = await generarPDFReservaBuffer(req.params.id, req.user?.userId);
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', `inline; filename="Reserva_${req.params.id}.pdf"`);
    res.send(pdfBuffer);
  } catch(e) {
...
app.get('/cron/verificar', async (req, res) => {
  try {
    await cronVerificarReservas(true);
    res.json({ ok: true, mensaje: 'Cron ejecutado' });
  } catch(e) { res.json({ ok: false, error: e.message }); }
});
...
async function cronVerificarReservas(manual = false) {
  if (!db) return;
  try {
    // Siempre verifica CREADAS y EMITIDAS (manual o automático)
    const estadoFilter = "estado IN ('CREADA','EMITIDA')";
    const intervaloFilter = manual ? '' : "AND (ultimo_check_cron IS NULL OR ultimo_check_cron < NOW() - INTERVAL '6 hours')";
    const r = await db.query(`
      SELECT id, pnr, order_id, quotation_id, estado, gds, notas, fecha_salida, aerolinea, usuario_id,
             origen, destino, itinerario_json, segmentos_json, adultos, ninos, infantes,
             COALESCE(pasajeros_json::json->0->>'apellido','') || ', ' || COALESCE(pasajeros_json::json->0->>'nombre','') as pasajero_nombre
      FROM reservas 
      WHERE ${estadoFilter}
        AND order_id IS NOT NULL 
        ${intervaloFilter}
        AND (fecha_salida IS NULL OR fecha_salida::timestamptz > NOW() - INTERVAL '12 months')
        AND (cron_finalizado IS NULL OR cron_finalizado = false)
    ...
    for (const reserva of r.rows) {
      try {
        // Verificar si el viaje ya terminó (último tramo)
        try {
          // PRIMERO: si fecha_salida está en el futuro, NUNCA marcar como finalizado
          if (reserva.fecha_salida && new Date(reserva.fecha_salida) > new Date()) {
            // El viaje todavía no empezó, seguir normalmente
          } else {
            const itin = typeof reserva.itinerario_json === 'string'
              ? JSON.parse(reserva.itinerario_json) : (reserva.itinerario_json || []);
            if (itin?.length) {
              // Buscar la última fecha de llegada en todos los segmentos
              let ultimaFecha = null;
              for (const leg of itin) {
                const segs = leg.segmentos?.length ? leg.segmentos : [leg];
                for (const s of segs) {
                  const f = s.llegada || s.arrivalDateTime || s.arrivalDate || '';
                  if (f) {
                    const t = new Date(f).getTime();
                    if (!isNaN(t) && (!ultimaFecha || t > ultimaFecha)) ultimaFecha = t;
                  }
                }
              }
              // Solo marcar como finalizado si tenemos una fecha concreta de llegada
              // Y esa fecha ya pasó hace más de 2 días
              if (ultimaFecha && ultimaFecha < Date.now() - 2 * 24 * 60 * 60 * 1000) {
                console.log(`[Cron-Verify] ${reserva.pnr}: viaje finalizado (último tramo: ${new Date(ultimaFecha).toLocaleDateString('es-AR')}), marcando cron_finalizado`);
                await db.query('UPDATE reservas SET cron_finalizado=true WHERE id=$1', [reserva.id]);
                continue;
              }
              // Si no encontramos fecha de llegada válida → NO marcar, seguir revisando
              if (!ultimaFecha) {
                console.log(`[Cron-Verify] ${reserva.pnr}: sin fecha de llegada en itinerario, no se marca como finalizado`);
              }
            }
          }
        } catch(e) {}
        ...
        // Determinar proveedor — quotation_id es más confiable que gds
        const isGEA_byQuotation = (reserva.quotation_id || '').startsWith('lleego_');
        const isSabre_byQuotation = (reserva.quotation_id || '').startsWith('sabre_');
        const isSabre = !isGEA_byQuotation && (
          isSabre_byQuotation || reserva.gds === 'Sabre'
        );
        const isGEA = isGEA_byQuotation || (
          !isSabre && (
            (reserva.gds && (reserva.gds.includes('NDC') || reserva.gds.includes('Lleego') || reserva.gds.includes('GEA'))) ||
            (reserva.notas && reserva.notas.includes('GEA'))
          )
        );
        // isTucano = cualquier otra cosa (GLAS, Tucano, NDC Tucano)
        
        let apiEstado = null;
        let mensaje = '';
        let detalle = {};

        // ── Sabre: no tiene API de retrieve disponible, skip verificación de segmentos ──
        if (isSabre) {
          console.log(`[Cron] ${reserva.pnr}: reserva Sabre, skip verify (sin API retrieve)`);
          await db.query('UPDATE reservas SET ultimo_check_cron=NOW() WHERE id=$1', [reserva.id]);
          continue;
        }
        ...
        if (isGEA) {
          // ── GEA verify ──
          const llToken = await getUserLleegoToken(reserva.usuario_id).catch(() => null);
          if (!llToken) { console.log(`[Cron] Sin credenciales GEA para reserva ${reserva.pnr}`); continue; }
          ...
          detalle.segmentosGEA = segments.map(s => ({
            flightNumber: `${s.marketing_company || ''}${s.transport_number || ''}`,
            departureAirportCode: s.departure, arrivalAirportCode: s.arrival,
            departureDateTime: s.std || s.departure_datetime,
            arrivalDateTime: s.sta || s.arrival_datetime,
            status: s.status
          }));
          ...
          // ── Tucano verify ──
          const token = await getUserTucanoToken(reserva.usuario_id).catch(() => null);
          if (!token) { console.log(`[Cron] Sin credenciales Tucano para reserva ${reserva.pnr}`); continue; }
          const hdrs = getHeaders(token);
          ...
          if (!resp.ok) {
            ...
            detalle.vuelos = vuelos; // guardar para comparación IA
            const vuelosCancelados = vuelos.filter(v => ['XX','UC','UN','HX','NO'].includes(v.status));
        ...
        // ── Detectar cambios granulares en itinerario ──
        let cambiosItinerario = [];
        const vuelosParaComparar = isGEA
          ? (detalle?.segmentosGEA || [])
          : (detalle?.vuelos || []);

        if (vuelosParaComparar.length) {
          // Preferir segmentos_json (tiene números de vuelo individuales)
          // Fallback a itinerario_json si segmentos_json está vacío o sin datos de vuelo
          let referencia = null;
          const segJson = reserva.segmentos_json;
          const segsGuardados = segJson
            ? (typeof segJson === 'string' ? JSON.parse(segJson) : segJson)
            : [];

          const tieneNumeros = segsGuardados.some(s => s.vuelo || s.flightNumber);

          if (tieneNumeros) {
            // Usar segmentos_json — lo envolvemos como array de legs para que detectarCambiosItinerario lo aplane
            referencia = segsGuardados.map(s => ({ segmentos: [s], ...s }));
            console.log(`[Cron-Detect] ${reserva.pnr}: usando segmentos_json (${segsGuardados.length} vuelos)`);
          } else if (reserva.itinerario_json) {
            // No tenemos números de vuelo → auto-calibrar guardando los datos actuales de la API como baseline
            // Esto ocurre para reservas viejas. Guardamos y NO reportamos cambios esta vez.
            const segmentosBaseline = vuelosParaComparar.map(v => ({
              vuelo: v.flightNumber || '',
              origen: v.departureAirportCode || v.origin || '',
              destino: v.arrivalAirportCode || v.destination || '',
              salida: v.departureDateTime || v.departureDate || '',
              llegada: v.arrivalDateTime || v.arrivalDate || '',
              aerolinea: (v.flightNumber || '').substring(0, 2)
            })).filter(s => s.vuelo || s.origen);
            if (segmentosBaseline.length) {
              await db.query('UPDATE reservas SET segmentos_json=$1 WHERE id=$2',
                [JSON.stringify(segmentosBaseline), reserva.id]);
              console.log(`[Cron-Detect] ${reserva.pnr}: baseline guardado (${segmentosBaseline.map(s=>s.vuelo).join(', ')}), próxima corrida comparará`);
            }
            referencia = null; // Esta corrida no compara, solo calibra
          }

          if (referencia) {
            cambiosItinerario = detectarCambiosItinerario(referencia, vuelosParaComparar);
          }
        }

        // Determinar si hay algo que reportar
        const estadoCambio   = apiEstado && apiEstado !== estadoAnterior;
        const hayCambios     = cambiosItinerario.length > 0 || (mensaje && !estadoCambio);
        const esReservaCaida = estadoCambio && (apiEstado === 'CANCELADA');
        ...
        // ── Procesar con IA si hay cambios o reserva caída ──
        if (estadoCambio || hayCambios) {
          try {
            const contactoReserva = await getContactoUsuario(reserva.usuario_id);
            const tipoEvento = esReservaCaida
              ? (estadoAnterior === 'EMITIDA' ? 'EMITIDA_CANCELADA' : 'RESERVA_CANCELADA')
              : cambiosItinerario.length ? 'CAMBIO_ITINERARIO' : 'CAMBIO_ESTADO';

            // ── Anti-repetición: primera vez → email + notificación. Repetición → solo notificación interna ──
            const cambiosHash = cambiosItinerario
              .map(c => `${c.tipo}|${c.segmento}|${c.diffMin || ''}|${c.status || ''}`)
              .sort().join(';;');
            const yaNotificado = await db.query(
              `SELECT id FROM notificaciones 
               WHERE reserva_id=$1 AND tipo='VUELO_CAMBIO' 
               AND detalle->>'cambiosHash' = $2
               AND created_at > NOW() - INTERVAL '24 hours'`,
              [reserva.id, cambiosHash]
            );
            const esPrimeraVez = !yaNotificado.rows.length || esReservaCaida;

            // Buscar alternativas si corresponde
            let alternativas = [];
            const necesitaAlternativas = esReservaCaida ||
              cambiosItinerario.some(c => c.tipo === 'CANCELADO') ||
              cambiosItinerario.some(c => c.tipo === 'HORARIO' && (Math.abs(c.diffMin) > 60 || (reserva.fecha_salida && (new Date(reserva.fecha_salida) - new Date()) < 7*24*60*60*1000)));

            if (necesitaAlternativas) {
              console.log(`[Cron-IA] Buscando alternativas para ${reserva.pnr}...`);
              alternativas = await Promise.race([
                buscarAlternativasCron(reserva, reserva.usuario_id),
                new Promise(r => setTimeout(() => r([]), 20000))
              ]);
              console.log(`[Cron-IA] ${alternativas.length} alternativas encontradas para ${reserva.pnr}`);
            }

            // Combinar cambios detectados con mensaje del cron
            const todosCambios = [
              ...cambiosItinerario,
              ...(mensaje && !cambiosItinerario.length ? [{ tipo: 'ESTADO', descripcion: mensaje }] : [])
            ];

            // IA analiza y redacta (solo si es primera vez)
            if (esPrimeraVez) {
              const analisis = await analizarCambioConIA({
                reserva, cambios: todosCambios, alternativas, tipoEvento, contacto: contactoReserva
              });
              await enviarEmailCambioIA(reserva, analisis, contactoReserva);
            } else {
              console.log(`[Cron-IA] ${reserva.pnr}: mismos cambios ya notificados por email, solo notificación interna`);
            }

            // Siempre registrar notificación interna (para que aparezca en el sistema)
            const notifTipo = hayCambios ? 'VUELO_CAMBIO' : 'ESTADO_CAMBIO';
            const notifMsg  = cambiosItinerario.length
              ? `${cambiosItinerario.length} cambio(s) en itinerario${esPrimeraVez ? '' : ' (ya notificado por email)'}`
              : mensaje || `Estado: ${apiEstado}`;
            await db.query(`INSERT INTO notificaciones (reserva_id, pnr, tipo, mensaje, detalle) VALUES ($1,$2,$3,$4,$5)`,
              [reserva.id, reserva.pnr, notifTipo, notifMsg,
               JSON.stringify({ cambios: todosCambios, alternativas, tipoEvento, cambiosHash, emailEnviado: esPrimeraVez })]);

          } catch(iaErr) {
            console.error(`[Cron-IA] Error procesando ${reserva.pnr}:`, iaErr.message);
          }
        }
...
// ─────────────────────────────────────────────────────────────
// IA DE MONITOREO — Detección de cambios + Búsqueda + Email IA
// ─────────────────────────────────────────────────────────────

// Normaliza número de vuelo: "MU 4918" → "MU4918", "MU0710" → "MU710" (sin ceros iniciales en número)
function normalizarVuelo(fn) {
  if (!fn) return '';
...
// Compara itinerario guardado en DB vs vuelos actuales de la API
function detectarCambiosItinerario(itinerarioGuardado, vuelosAPI) {
  const cambios = [];
  if (!itinerarioGuardado || !vuelosAPI?.length) return cambios;

  // Aplanar legs guardados (puede tener segmentos internos o no)
  const legsGuardados = [];
  for (const leg of itinerarioGuardado) {
    legsGuardados.push(leg);
  }
  if (!legsGuardados.length) return cambios;

  // ── Verificar vuelos cancelados (siempre, independiente de cómo guardamos) ──
  for (const v of vuelosAPI) {
    if (['XX','HX','UC','UN','NO'].includes(v.status)) {
      const fn  = v.flightNumber || `${v.marketingAirlineCode || ''}${v.flightNumber || ''}`;
      const dep = v.departureAirportCode || v.origin || '';
      const arr = v.arrivalAirportCode   || v.destination || '';
      cambios.push({ tipo: 'CANCELADO', segmento: `${fn} ${dep}→${arr}`, status: v.status });
    }
  }
  // Si hay cancelados, no seguir analizando horarios (ya hay problema grave)
  if (cambios.length) return cambios;

  // ── Intentar comparación por número de vuelo (si los segmentos guardados tienen vuelo) ──
  const segsGuardados = [];
  for (const leg of legsGuardados) {
    if (leg.segmentos?.length) {
      for (const s of leg.segmentos) segsGuardados.push(s);
    } else {
      segsGuardados.push(leg);
    }
  }
  const tienenNumero = segsGuardados.some(s => s.vuelo || s.flightNumber);

  if (tienenNumero) {
    // Comparación granular por número de vuelo
    const mapaGuardados = {};
    for (const s of segsGuardados) {
      const key = normalizarVuelo(s.vuelo || s.flightNumber || '');
      if (!key) continue;
      if (!mapaGuardados[key]) mapaGuardados[key] = [];
      mapaGuardados[key].push(s);
    }
    for (const v of vuelosAPI) {
      const flightNumRaw = v.flightNumber || `${v.marketingAirlineCode || ''}${v.flightNumber || ''}`;
      const flightNum    = normalizarVuelo(flightNumRaw);
      const depIATA      = v.departureAirportCode || v.origin || '';
      const arrIATA      = v.arrivalAirportCode   || v.destination || '';
      const depAPI       = v.departureDateTime || v.departureDate || '';
      const segLabel     = `${flightNumRaw} ${depIATA}→${arrIATA}`;

      const candidatos = mapaGuardados[flightNum] || [];
      let g = candidatos.length === 1 ? candidatos[0]
            : candidatos.find(c => (c.origen || c.departureAirportCode || '') === depIATA) || candidatos[0];
      if (!g) continue; // vuelo nuevo, no es cambio

      const depG = g.salida || g.departureDateTime || '';
      if (depAPI && depG) {
        const tAPI = new Date(depAPI).getTime();
        const tG   = new Date(depG).getTime();
        if (!isNaN(tAPI) && !isNaN(tG) && Math.abs(tAPI - tG) > 5 * 60 * 1000) {
          cambios.push({ tipo: 'HORARIO', segmento: segLabel, diffMin: Math.round((tAPI-tG)/60000), original: depG, nuevo: depAPI });
        }
      }
      const origG = g.origen || g.departureAirportCode || '';
      const destG = g.destino || g.arrivalAirportCode   || '';
      if (depIATA && origG && depIATA !== origG)
        cambios.push({ tipo: 'AEROPUERTO', segmento: segLabel, campo: 'origen', original: origG, nuevo: depIATA });
      if (arrIATA && destG && arrIATA !== destG)
        cambios.push({ tipo: 'AEROPUERTO', segmento: segLabel, campo: 'destino', original: destG, nuevo: arrIATA });
    }
    return cambios;
  }

  // ── Fallback: itinerario guardado solo tiene legs (sin números de vuelo) ──
  // Agrupar vuelos de la API en legs: el primer vuelo cuyo origen coincide con el leg guardado
  // inicia ese leg; termina cuando el destino coincide con el destino del leg guardado.
  for (const leg of legsGuardados) {
    const legOrigen  = leg.origen  || leg.departureAirportCode || '';
    const legDestino = leg.destino || leg.arrivalAirportCode   || '';
    const legSalida  = leg.salida  || leg.departureDateTime    || '';
    const legLlegada = leg.llegada || leg.arrivalDateTime      || '';
    if (!legOrigen || !legDestino) continue;

    // Buscar el primer vuelo de la API que sale del mismo origen
    const primerVuelo = vuelosAPI.find(v =>
      (v.departureAirportCode || v.origin || '') === legOrigen
    );
    // Buscar el último vuelo que llega al mismo destino
    const ultimoVuelo = [...vuelosAPI].reverse().find(v =>
      (v.arrivalAirportCode || v.destination || '') === legDestino
    );

    if (!primerVuelo && !ultimoVuelo) continue; // este leg no está en la API, skip

    const legLabel = `${legOrigen}→${legDestino}`;

    // Comparar salida del primer vuelo vs salida guardada del leg
    if (primerVuelo && legSalida) {
      const depAPI = primerVuelo.departureDateTime || primerVuelo.departureDate || '';
      if (depAPI) {
        const tAPI = new Date(depAPI).getTime();
        const tG   = new Date(legSalida).getTime();
        if (!isNaN(tAPI) && !isNaN(tG) && Math.abs(tAPI - tG) > 5 * 60 * 1000) {
          const fn = primerVuelo.flightNumber || `${primerVuelo.marketingAirlineCode || ''}${primerVuelo.flightNumber || ''}`;
          cambios.push({ tipo: 'HORARIO', segmento: `${fn} ${legLabel}`, diffMin: Math.round((tAPI-tG)/60000), original: legSalida, nuevo: depAPI });
        }
      }
    }

    // Comparar llegada del último vuelo vs llegada guardada del leg
    if (ultimoVuelo && legLlegada) {
      const arrAPI = ultimoVuelo.arrivalDateTime || ultimoVuelo.arrivalDate || '';
      if (arrAPI) {
        const tAPI = new Date(arrAPI).getTime();
        const tG   = new Date(legLlegada).getTime();
        if (!isNaN(tAPI) && !isNaN(tG) && Math.abs(tAPI - tG) > 5 * 60 * 1000) {
          const fn = ultimoVuelo.flightNumber || `${ultimoVuelo.marketingAirlineCode || ''}${ultimoVuelo.flightNumber || ''}`;
          cambios.push({ tipo: 'HORARIO', segmento: `${fn} ${legLabel} (llegada)`, diffMin: Math.round((tAPI-tG)/60000), original: legLlegada, nuevo: arrAPI });
        }
      }
    }
  }
...
// Busca alternativas en Tucano/GEA para una reserva dada
async function buscarAlternativasCron(reserva, userId) {
  const alternativas = [];
  try {
    const itin = typeof reserva.itinerario_json === 'string'
      ? JSON.parse(reserva.itinerario_json) : (reserva.itinerario_json || []);
    if (!itin?.length) return alternativas;

    // Armar parámetros de búsqueda desde el itinerario guardado
    const legs = [];
    for (const leg of itin) {
      const origen  = leg.origen  || leg.departureAirportCode || '';
      const destino = leg.destino || leg.arrivalAirportCode   || '';
      let salida = leg.salida || leg.departureDateTime || leg.departureDate || '';
      if (salida) salida = salida.substring(0, 10); // YYYY-MM-DD
      if (origen && destino && salida) legs.push({ origen, destino, salida });
    }
    if (!legs.length) return alternativas;

    const tipo       = legs.length === 1 ? 'oneway' : (legs.length === 2 && legs[0].origen === legs[1].destino ? 'roundtrip' : 'multidestino');
    const adultos    = reserva.adultos || 1;
    const ninos      = reserva.ninos   || 0;
    const infantes   = reserva.infantes || 0;
    const cabinType  = 0; // economy por defecto
    const searchParams = {
      tipo, origen: legs[0].origen, destino: legs[0].destino,
      salida: legs[0].salida,
      regreso: tipo === 'roundtrip' ? legs[1].salida : undefined,
      tramos: tipo === 'multidestino' ? legs : undefined,
      adultos, ninos, infantes, cabinType, stops: null, airlines: []
    };

    // Buscar en Tucano (GLAS) y GEA en paralelo
    const [tucanoRes, geaRes] = await Promise.allSettled([
      (async () => {
        const token = await getUserTucanoToken(userId).catch(() => null);
        if (!token) return [];
        // Reusar la lógica de búsqueda GLAS simplificada
        const stopsFilter = null;
        const glasCabinVal = 0;
        let payload, endpoint, addSearchPayload;
        if (tipo === 'oneway') {
          endpoint = `${API_BASE}/FlightSearch/OnewayRemake`;
          payload = { DepartCode: legs[0].origen, ArrivalCode: legs[0].destino, DepartDate: `${legs[0].salida}T00:00:00`, DepartTime: null, Adults: adultos, Childs: ninos, Infants: infantes, CabinType: glasCabinVal, Stops: null, Airlines: [], TypeOfFlightAllowedInItinerary: null, SortByGLASAlgorithm: '', AlternateCurrencyCode: 'USD', CorporationCodeGlas: null, IncludeFiltersOptions: true };
          addSearchPayload = { SearchTravelType: 2, OneWayModel: payload, MultipleLegsModel: null, RoundTripModel: null };
        } else if (tipo === 'roundtrip') {
          endpoint = `${API_BASE}/FlightSearch/RoundTripRemake`;
          payload = { DepartCode: legs[0].origen, ArrivalCode: legs[0].destino, DepartDate: `${legs[0].salida}T00:00:00`, ArrivalDate: `${legs[1].salida}T00:00:00`, ArrivalTime: null, DepartTime: null, Adults: adultos, Childs: ninos, Infants: infantes, CabinType: glasCabinVal, Stops: stopsFilter, Airlines: [], TypeOfFlightAllowedInItinerary: null, SortByGLASAlgorithm: '', AlternateCurrencyCode: 'USD', CorporationCodeGlas: null, IncludeFiltersOptions: true };
          addSearchPayload = { SearchTravelType: 1, OneWayModel: null, MultipleLegsModel: null, RoundTripModel: payload };
        } else {
          return []; // multidestino skip por ahora
        }
        const hdrs = getHeaders(token);
        const searchRes = await fetch(`${API_BASE}/FlightSearch/AddSearch`, { method: 'POST', headers: hdrs, body: JSON.stringify(addSearchPayload) });
        if (!searchRes.ok) return [];
        const searchData = await searchRes.json();
        const searchId = searchData?.searchId || searchData?.SearchId;
        if (!searchId) return [];
        await new Promise(r => setTimeout(r, 3000));
        const resultsRes = await fetch(`${API_BASE}/FlightSearch/GetResults?searchId=${searchId}`, { headers: hdrs });
        if (!resultsRes.ok) return [];
        const resultsData = await resultsRes.json();
        const quotations = resultsData?.quotations || resultsData?.Quotations || [];
        return quotations.slice(0, 5).map(q => ({
          fuente: 'Tucano',
          aerolinea: q.airline || q.Airline || '',
          precio: q.totalPrice || q.TotalPrice || q.totalUSD || 0,
          moneda: 'USD',
          itinerario: (q.legs || q.Legs || []).map(l => `${l.departureCode || l.DepartCode}→${l.arrivalCode || l.ArrivalCode} ${(l.departureDateTime || l.DepartDate || '').substring(11,16)} → ${(l.arrivalDateTime || l.ArrivalDate || '').substring(11,16)}`)
        }));
      })(),
      (async () => {
        const token = await getUserLleegoToken(userId).catch(() => null);
        if (!token) return [];
        const geaParams = {
          tipo, origen: legs[0].origen, destino: legs[0].destino,
          salida: legs[0].salida,
          regreso: tipo === 'roundtrip' ? legs[1].salida : null,
          adultos, ninos, infantes, cabinType: 0, stops: null, tramos: legs, airlines: []
        };
        const geaResults = await buscarLleego(geaParams).catch(() => []);
        return geaResults.slice(0, 5).map(r => ({
          fuente: 'GEA',
          aerolinea: r.aerolinea || '',
          precio: r.precioUSD || r.precioTotal || 0,
          moneda: 'USD',
          itinerario: (r.itinerario || []).map(l => `${l.origen}→${l.destino} ${(l.salida||'').substring(11,16)} → ${(l.llegada||'').substring(11,16)}`)
        }));
      })()
    ]);

    if (tucanoRes.status === 'fulfilled') alternativas.push(...(tucanoRes.value || []));
    if (geaRes.status   === 'fulfilled') alternativas.push(...(geaRes.value   || []));

    // Ordenar por precio y tomar las 3 mejores
    alternativas.sort((a, b) => (a.precio || 9999) - (b.precio || 9999));
    return alternativas.slice(0, 3);
  } catch(e) {
    console.error('[BuscarAlternativas] Error:', e.message);
    return [];
  }
}

// Llama a Claude para analizar el cambio y redactar el email
async function analizarCambioConIA({ reserva, cambios, alternativas, tipoEvento, contacto }) {
  const ANTHROPIC_API_KEY = process.env.ANTHROPIC_API_KEY;
  if (!ANTHROPIC_API_KEY) return null;

  const diasAlViaje = reserva.fecha_salida
    ? Math.round((new Date(reserva.fecha_salida) - new Date()) / (1000*60*60*24))
    : null;

  const altTexto = alternativas.length
    ? alternativas.map((a, i) => `${i+1}. ${a.aerolinea} (${a.fuente}) — USD ${a.precio} — ${a.itinerario?.join(' / ')}`).join('\n')
    : 'No se encontraron alternativas disponibles en esta ruta.';

  const cambiosTexto = cambios.map(c => {
    if (c.tipo === 'HORARIO') return `Cambio de horario en ${c.segmento}: ${c.original} → ${c.nuevo} (${c.diffMin > 0 ? '+' : ''}${c.diffMin} min)`;
    if (c.tipo === 'CANCELADO') return `Vuelo cancelado: ${c.segmento} (status: ${c.status})`;
    if (c.tipo === 'AEROPUERTO') return `Cambio de aeropuerto en ${c.segmento}: ${c.campo} cambió de ${c.original} a ${c.nuevo}`;
    if (c.tipo === 'VUELO') return `Cambio de número de vuelo en ${c.segmento}: ${c.original} → ${c.nuevo}`;
    return JSON.stringify(c);
  }).join('\n');

  const prompt = `Sos asistente de una agencia de viajes argentina. Analizá el siguiente cambio en una reserva y redactá un email conciso y profesional en español para el vendedor.

RESERVA: ${reserva.pnr}
PASAJERO(S): ${reserva.pasajero_nombre || '(ver reserva)'}
RUTA: ${reserva.origen || ''} → ${reserva.destino || ''}
FECHA DE VIAJE: ${reserva.fecha_salida ? new Date(reserva.fecha_salida).toLocaleDateString('es-AR') : 'no especificada'}
DÍAS HASTA EL VIAJE: ${diasAlViaje !== null ? diasAlViaje : 'desconocido'}
...
- Asunto: corto y claro, con el PNR
- Cuerpo: describí el cambio, evaluá el impacto (urgente/importante/informativo), y presentá las alternativas si las hay
- Si el viaje es en menos de 7 días, marcalo como urgente
- Si hay vuelos cancelados con ticket emitido, indicar que hay que contactar al proveedor
- Tono profesional pero directo, sin exceso de formalidad
- Respondé SOLO con JSON en este formato exacto:
{"asunto":"...","cuerpo":"...","urgencia":"alta|media|baja"}`;
  ...
  try {
    ...
    console.error('[IA] Error al analizar cambio:', e.message);
    return null;
  }
}

// Envía el email de cambio redactado por la IA
async function enviarEmailCambioIA(reserva, analisis, contacto) {
  if (!resendClient || !analisis) return;
  const fromField = contacto ? `${contacto.nombre} <${contacto.mail}>` : 'Lucky Tour <guido@luckytourviajes.com>';
  const toField   = contacto?.mail || 'guido@luckytourviajes.com';
  const urgenciaColor = analisis.urgencia === 'alta' ? '#c0392b' : analisis.urgencia === 'media' ? '#e67e22' : '#2980b9';
  const urgenciaLabel = analisis.urgencia === 'alta' ? '🔴 URGENTE' : analisis.urgencia === 'media' ? '🟠 IMPORTANTE' : '🔵 INFORMATIVO';

  const html = `
    <div style="font-family:Arial,sans-serif;max-width:640px;margin:0 auto;background:#f5f5f5;padding:20px;">
      <div style="background:${urgenciaColor};color:white;padding:12px 20px;border-radius:6px 6px 0 0;font-size:13px;font-weight:bold;">
        ${urgenciaLabel} — Cambio en Reserva ${reserva.pnr}
      </div>
      <div style="background:white;padding:24px;border-radius:0 0 6px 6px;white-space:pre-wrap;font-size:14px;line-height:1.6;color:#333;">
        ${analisis.cuerpo.replace(/\n/g, '<br>')}
      </div>
      <p style="color:#aaa;font-size:11px;text-align:center;margin-top:12px;">Lucky Tour — Sistema de monitoreo automático</p>
    </div>`;
  ...
  try {
    await resendClient.emails.send({
      from: fromField,
      to: toField,
      subject: analisis.asunto,
      html
    });
    console.log(`[IA-Email] Enviado para ${reserva.pnr}: ${analisis.asunto}`);
  } catch(e) {
    console.error('[IA-Email] Error:', e.message);
  }
}
...
// ─────────────────────────────────────────────────────────────
// MONITOR TLV — Búsqueda constante de disponibilidad Israel
// ─────────────────────────────────────────────────────────────
const TLV_DESTINOS = [
  { iata: 'FCO', nombre: 'Roma' },
  { iata: 'MXP', nombre: 'Milán' },
  { iata: 'AMS', nombre: 'Amsterdam' },
  { iata: 'MAD', nombre: 'Madrid' },
  { iata: 'BCN', nombre: 'Barcelona' },
  { iata: 'FRA', nombre: 'Frankfurt' },
  { iata: 'CDG', nombre: 'París' },
  { iata: 'LHR', nombre: 'Londres' },
  { iata: 'PRG', nombre: 'Praga' },
  { iata: 'ATH', nombre: 'Atenas' },
  { iata: 'LIS', nombre: 'Lisboa' },
  { iata: 'ZRH', nombre: 'Zurich' },
  { iata: 'GVA', nombre: 'Ginebra' },
];
const TLV_FECHAS = ['2026-03-09', '2026-03-10', '2026-03-11', '2026-03-12'];
const TLV_AEROLINEAS = ['IZ', 'IS', 'LY']; // Arkia, Israir, El Al
const TLV_EMAIL_TO = ['guido@luckytourviajes.com'];
let tlvMonitorActivo = true;

async function cronMonitorTLV() {
  // Apagar automáticamente después del 12/03/2026
  const hoy = new Date().toISOString().substring(0, 10);
  if (hoy >= '2026-03-12') {
    if (tlvMonitorActivo) {
      tlvMonitorActivo = false;
      console.log('[TLV] Monitor apagado automáticamente — fechas de búsqueda vencidas');
    }
    return;
  }
  if (!tlvMonitorActivo) return;
  console.log(`[TLV] ===== Iniciando búsqueda (${new Date().toLocaleTimeString('es-AR')}) =====`);
  console.log(`[TLV] Fechas: ${TLV_FECHAS.join(', ')} | Destinos: ${TLV_DESTINOS.map(d=>d.iata).join(', ')}`);
  console.log(`[TLV] Aerolíneas: ${TLV_AEROLINEAS.join(', ')} (Arkia/Israir/El Al)`);
  
  const encontrados = [];

  // Armar todas las combinaciones fecha × destino
  const combinaciones = [];
  for (const fecha of TLV_FECHAS) {
    for (const dest of TLV_DESTINOS) {
      combinaciones.push({ fecha, dest });
    }
  }
  console.log(`[TLV] ${combinaciones.length} combinaciones a verificar`);

  // Procesar de a 4 en paralelo para no saturar las APIs
  const BATCH = 4;
  for (let i = 0; i < combinaciones.length; i += BATCH) {
    const batch = combinaciones.slice(i, i + BATCH);
    await Promise.all(batch.map(async ({ fecha, dest }) => {
      try {
        // Tucano da 400 en rutas TLV — desactivado por ahora
        // Solo buscar en GEA/Lleego
        const llToken = await getUserLleegoToken(1).catch(() => null);
        if (llToken) {
          console.log(`[TLV] GEA buscando TLV→${dest.iata} ${fecha}...`);
          const body = {
            query: {
              criterias: [{
                rule: { combined: false, duplicated: false, show_data: true, show_partial: false, only_partial: false },
                travel: {
                  ...
                  include_train: false, include_bus: false,
                  include_gds: true, include_ndc: true, low_cost: false,
                  only_flight: true, exclude_fares: ['CUPO'],
                  journeys: [{ origin: 'TLV', destination: dest.iata, date: fecha, max_layover_count: 0 }],
                  paxes_distribution: { passengers_ages: [30] }
                }
              }]
            }
          };
          const lr = await fetch('https://api-tr.lleego.com/api/v2/transport/avail?locale=es-ar', {
            method: 'POST',
            headers: { 'Authorization': `Bearer ${llToken}`, 'x-api-key': LLEEGO_API_KEY, 'Content-Type': 'application/json', 'lang': 'es-ar' },
            body: JSON.stringify(body)
          }).catch(() => null);
          if (lr?.ok) {
            const lData = await lr.json().catch(() => null);
            const sols = lData?.solutions || [];
            console.log(`[TLV] GEA TLV→${dest.iata} ${fecha}: ${sols.length} soluciones totales`);
            if (sols.length > 0) {
              console.log(`[TLV] GEA sol[0] keys:`, Object.keys(sols[0]).join(', '));
              console.log(`[TLV] GEA sol[0] sample:`, JSON.stringify(sols[0]).substring(0, 400));
              // Log primer segmento para encontrar campo de plazas
              const segsObj = lData?.segments || {};
              const firstSegKey = Object.keys(segsObj)[0];
              if (firstSegKey) console.log(`[TLV] GEA seg[0]:`, JSON.stringify(segsObj[firstSegKey]).substring(0, 400));
            }
            // Filtrar por aerolínea buscando en los journeys/segments
            const segments = lData?.segments || {};
            const journeys = lData?.journeys || {};
            const filtGEA = sols.filter(sol => {
              const assocs = sol.data?.associations || [];
              for (const assoc of assocs) {
                const jRefs = assoc.journey_references || [];
                for (const jRef of jRefs) {
                  const journey = journeys[jRef];
                  if (!journey) continue;
                  for (const segId of (journey.segments || [])) {
                    const seg = segments[segId];
                    if (seg && TLV_AEROLINEAS.includes((seg.marketing_company || '').toUpperCase())) return true;
                  }
                }
                const vc = (assoc.validating_company || '').toUpperCase();
                if (TLV_AEROLINEAS.includes(vc)) return true;
              }
              return false;
            });
            console.log(`[TLV] GEA TLV→${dest.iata} ${fecha}: ${filtGEA.length} con IZ/IS/LY`);
            if (filtGEA.length) {
              const vuelosInfo = filtGEA.slice(0, 5).map(sol => {
                const assocs = sol.data?.associations || [];
                const aerolineasSol = new Set();
                let salida = null, llegada = null, nroVuelo = null, seats = null;
                assocs.forEach(assoc => {
                  (assoc.journey_references || []).forEach(jRef => {
                    const journey = journeys[jRef];
                    const segs = journey?.segments || [];
                    if (segs.length) {
                      const firstSeg = segments[segs[0]];
                      const lastSeg = segments[segs[segs.length - 1]];
                      if (firstSeg) {
                        if (!salida) salida = firstSeg.departure_date;
                        if (!nroVuelo) nroVuelo = `${firstSeg.marketing_company}${firstSeg.transport_number}`;
                      }
                      if (lastSeg && !llegada) llegada = lastSeg.arrival_date;
                      segs.forEach(segId => {
                        const seg = segments[segId];
                        if (seg?.marketing_company) aerolineasSol.add(seg.marketing_company);
                        // Buscar seats en segment_references
                        const segRef = assoc.segment_references?.[segId];
                        if (segRef?.seats != null && (seats === null || segRef.seats < seats)) {
                          seats = segRef.seats; // tomar el mínimo (el más restrictivo)
                        }
                      });
                    }
                  });
                });
                const salidaFmt = salida ? salida.substring(11, 16) : null;
                const llegadaFmt = llegada ? llegada.substring(11, 16) : null;
                return {
                  aerolinea: [...aerolineasSol].join('/'),
                  precio: sol.total_price?.total || 0,
                  vuelo: nroVuelo,
                  salida: salidaFmt,
                  llegada: llegadaFmt,
                  seats
              ...
              encontrados.push({
                fecha, destino: dest, fuente: 'GEA',
                cantidad: filtGEA.length,
                vuelos: vuelosInfo
              });
            }
          } else {
            console.log(`[TLV] GEA TLV→${dest.iata} ${fecha}: sin respuesta (status ${lr?.status || 'null'})`);
          }
        }
      } catch(e) {
        console.log(`[TLV] Error ${dest.iata} ${fecha}: ${e.message}`);
      }
    }));
    // Pequeña pausa entre batches
    if (i + BATCH < combinaciones.length) await new Promise(r => setTimeout(r, 1000));
  }

  if (encontrados.length) {
    console.log(`[TLV] ✅ ${encontrados.length} combinaciones con disponibilidad:`);
    for (const e of encontrados) {
      const aerolineas = e.vuelos.map(v => v.aerolinea).filter(Boolean).join(', ');
      console.log(`[TLV]  → ${e.fecha} TLV→${e.destino.iata} (${e.fuente}): ${e.cantidad} lugar/es | Aerolíneas: ${aerolineas || '?'}`);
    }
    await enviarEmailTLV(encontrados);
  } else {
    console.log(`[TLV] ===== Sin disponibilidad en esta corrida =====`);
  }
}

async function enviarEmailTLV(encontrados) {
  try {
    // Obtener suscriptores activos de la DB
    let destinatarios = [{ nombre: 'Guido', email: 'guido@luckytourviajes.com' }];
    if (db) {
      try {
        const r = await db.query('SELECT nombre, email FROM tlv_suscriptores WHERE activo=true');
        if (r.rows.length) destinatarios = r.rows;
      } catch(e) { console.log('[TLV] Error leyendo suscriptores:', e.message); }
    }
    console.log(`[TLV] Enviando a ${destinatarios.length} suscriptores`);
    const porFecha = {};
    for (const e of encontrados) {
      const k = e.fecha;
    ...
    let html = `
      <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto">
        ...
        <div style="background:#c9a84c;padding:16px 20px;border-radius:0">
          ...
          <p style="color:#333;margin:4px 0 0;font-size:13px">${new Date().toLocaleString('es-AR', {timeZone:'America/Argentina/Buenos_Aires'})}</p>
        </div>
    ...
    for (const [fecha, items] of Object.entries(porFecha)) {
      ...
      html += `<h3 style="color:#c9a84c;margin:16px 0 8px;font-size:15px;border-bottom:1px solid #333;padding-bottom:6px">${fechaFmt}</h3>`;
      for (const e of items) {
        const vuelosStr = e.vuelos.map(v => {
          const precioBase = v.precio || 0;
          const precioFinal = Math.ceil(precioBase + 40);
          const horariosStr = (v.salida && v.llegada) ? `${v.salida} → ${v.llegada}` : '';
          const seatsStr = v.seats != null ? `${v.seats} lugar${v.seats !== 1 ? 'es' : ''}` : '';
          const waMensaje = encodeURIComponent(`Hola, quiero este vuelo TLV → ${e.destino.iata} (${e.destino.nombre}) del ${fechaFmt}${v.vuelo ? `. Vuelo: ${v.vuelo}` : ''}${horariosStr ? `. Horario: ${horariosStr}` : ''}. Necesito X cantidad de lugares. Por favor reservar. Te envío la foto del pasaporte.`);
          const waUrl = `https://wa.me/5491126301836?text=${waMensaje}`;
          return `
            <div style="display:flex;justify-content:space-between;align-items:center;padding:10px 0;border-bottom:1px solid #1e1e2e">
              <div>
                <div style="display:flex;align-items:center;gap:8px;margin-bottom:5px">
                  <span style="color:#fff;font-size:14px;font-weight:700">TLV → ${e.destino.iata} (${e.destino.nombre})</span>
                  ${seatsStr ? `<span style="background:#2ecc71;color:#000;font-size:10px;font-weight:700;padding:2px 7px;border-radius:10px">${seatsStr}</span>` : ''}
                </div>
                <div style="color:#aaa;font-size:12px;margin-bottom:4px">${v.vuelo || ''}${horariosStr ? `&nbsp;&nbsp;${horariosStr}` : ''}</div>
                <div style="color:#c9a84c;font-size:13px;font-weight:700">USD ${precioFinal} <span style="color:#555;font-size:10px;font-weight:400">incl. carry on</span></div>
              </div>
              <a href="${waUrl}" style="background:#25D366;color:#fff;font-size:12px;font-weight:700;padding:6px 16px;border-radius:8px;text-decoration:none;white-space:nowrap;flex-shrink:0;margin-left:12px;align-self:center">Reservar</a>
            </div>`;
        }).join('');
        html += `
          <div style="background:#13131f;border:1px solid #333;border-left:3px solid #2ecc71;border-radius:6px;padding:10px 14px;margin-bottom:8px">
            ${vuelosStr}
          </div>`;
      }
    }
    ...
    html += `
        <p style="color:#888;font-size:11px;margin-top:16px;text-align:center">⚠️ Disponibilidad al momento del envío: ${new Date().toLocaleString('es-AR', {timeZone:'America/Argentina/Buenos_Aires'})}. Puede cambiar en minutos.</p>
        <p style="color:#555;font-size:11px;margin-top:6px;text-align:center">Monitor TLV — Lucky Tour · Se actualiza cada 3 minutos</p>
        <div style="margin-top:20px;border-top:1px solid #2a2a3e;padding-top:16px;text-align:center">
          <p style="color:#c9a84c;font-weight:700;font-size:13px;margin:0 0 6px">Guido Finkelstein</p>
          <p style="color:#888;font-size:12px;margin:2px 0">✉ guido@luckytourviajes.com</p>
          <p style="color:#888;font-size:12px;margin:2px 0">📱 +54 9 11 6846 3892</p>
        </div>
        </div>
      </div>`;

    const subject = `🚨 TLV Disponibilidad — ${encontrados.length} ruta${encontrados.length !== 1 ? 's' : ''} con lugares`;
    let enviados = 0;
    for (const dest of destinatarios) {
      try {
        await resendClient.emails.send({
          from: 'Lucky Tour Monitor <guido@luckytourviajes.com>',
          to: dest.email,
          subject,
          html
        });
        ...
        console.log(`[TLV] Error enviando a ${dest.email}: ${e.message}`);
      }
    }
    console.log(`[TLV] Email enviado a ${enviados}/${destinatarios.length} suscriptores`);
  } catch(e) {
    console.error('[TLV] Error email:', e.message);
  }
}

// Endpoint para activar/desactivar el monitor manualmente
// ─── TLV SUSCRIPTORES ───
app.get('/tlv-alertas', (req, res) => {
  res.send(`<!DOCTYPE html>
<html lang="es">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>Alertas vuelos Tel Aviv — Lucky Tour</title>
<style>
  *{box-sizing:border-box;margin:0;padding:0}
  body{font-family:'Segoe UI',Arial,sans-serif;background:#0f1117;min-height:100vh;display:flex;align-items:center;justify-content:center;padding:20px}
  .card{background:#1a1a2e;border:1px solid #252540;border-radius:16px;padding:36px 32px;max-width:420px;width:100%}
  .logo{text-align:center;margin-bottom:24px}
  .logo span{font-size:28px;font-weight:800;color:#c9a84c;letter-spacing:1px}
  .logo p{color:#555;font-size:13px;margin-top:4px}
  h1{color:#fff;font-size:20px;font-weight:700;margin-bottom:6px;text-align:center}
  .sub{color:#666;font-size:13px;text-align:center;margin-bottom:28px;line-height:1.5}
  .sub strong{color:#c9a84c}
  label{display:block;color:#888;font-size:12px;font-weight:600;text-transform:uppercase;letter-spacing:.8px;margin-bottom:6px}
  input{width:100%;background:#0f1117;border:1px solid #2a2a3e;border-radius:8px;padding:11px 14px;color:#fff;font-size:14px;outline:none;transition:border .2s;margin-bottom:16px}
  input:focus{border-color:#c9a84c}
  button{width:100%;background:#c9a84c;color:#000;font-weight:800;font-size:15px;border:none;border-radius:8px;padding:13px;cursor:pointer;transition:opacity .2s}
  button:hover{opacity:.85}
  button:disabled{opacity:.5;cursor:not-allowed}
  .msg{margin-top:16px;padding:12px 16px;border-radius:8px;font-size:13px;text-align:center;display:none}
  .msg.ok{background:#0d2d1a;color:#2ecc71;border:1px solid #2ecc71}
  .msg.err{background:#2d0d0d;color:#e74c3c;border:1px solid #e74c3c}
  .info{margin-top:20px;color:#444;font-size:11px;text-align:center;line-height:1.6}
</style>
</head>
<body>
<div class="card">
  <div class="logo"><span>✈ Lucky Tour</span><p>Sistema de alertas</p></div>
  <h1>Alertas vuelos Tel Aviv</h1>
  <p class="sub">Recibí un email cada vez que aparezca disponibilidad en vuelos directos desde <strong>Tel Aviv</strong> hacia Europa (9, 10 y 11 de marzo).</p>
  <div>
    <label>Tu nombre</label>
    <input type="text" id="nombre" placeholder="Ej: María García" autocomplete="name">
    <label>Tu email</label>
    <input type="email" id="email" placeholder="tu@email.com" autocomplete="email">
    <button id="btn" onclick="suscribir()">🔔 Quiero recibir alertas</button>
    <div class="msg" id="msg"></div>
  </div>
  <p class="info">Podés darte de baja en cualquier momento.<br>Solo enviamos alertas de disponibilidad real.</p>
</div>
<script>
async function suscribir(){
  ...
  if(!nombre || !email){ showMsg('Completá tu nombre y email.', false); return; }
  if(!/^[^@]+@[^@]+\\.[^@]+$/.test(email)){ showMsg('Email inválido.', false); return; }
  btn.disabled = true; btn.textContent = 'Suscribiendo...';
  try {
    const r = await fetch('/tlv-alertas/suscribir', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({nombre,email})});
    const d = await r.json();
    if(d.ok){ showMsg('\\u2705 ¡Listo! Te avisamos cuando haya lugares disponibles.', true); btn.textContent='\\u2705 Suscripto'; }
    else { showMsg(d.error || 'Error al suscribirse.', false); btn.disabled=false; btn.textContent='\\uD83D\\uDD14 Quiero recibir alertas'; }
  } catch(e){ showMsg('Error de conexión.', false); btn.disabled=false; btn.textContent='\\uD83D\\uDD14 Quiero recibir alertas'; }
}
function showMsg(txt, ok){
  const el = document.getElementById('msg');
  el.textContent = txt; el.className = 'msg ' + (ok?'ok':'err'); el.style.display='block';
}
document.getElementById('email').addEventListener('keydown', e => { if(e.key==='Enter') suscribir(); });
</script>
</body>
</html>`);
});

app.post('/tlv-alertas/suscribir', async (req, res) => {
  const { nombre, email } = req.body;
  if (!nombre || !email) return res.json({ ok: false, error: 'Nombre y email requeridos' });
  if (!/^[^@]+@[^@]+\.[^@]+$/.test(email)) return res.json({ ok: false, error: 'Email inválido' });
  if (!db) return res.json({ ok: false, error: 'DB no disponible' });
  try {
    await db.query(`INSERT INTO tlv_suscriptores (nombre, email) VALUES ($1, $2) ON CONFLICT (email) DO UPDATE SET nombre=$1, activo=true`, [nombre.trim(), email.trim().toLowerCase()]);
    console.log(`[TLV] Nuevo suscriptor: ${nombre} <${email}>`);
    res.json({ ok: true });
  } catch(e) { res.json({ ok: false, error: e.message }); }
});

app.get('/tlv-alertas/lista', async (req, res) => {
  if (!req.user) return res.status(401).json({ ok: false });
  try {
    const r = await db.query('SELECT * FROM tlv_suscriptores ORDER BY created_at DESC');
    res.json({ ok: true, suscriptores: r.rows });
  } catch(e) { res.json({ ok: false, error: e.message }); }
});

app.delete('/tlv-alertas/suscriptor/:id', async (req, res) => {
  if (!req.user) return res.status(401).json({ ok: false });
  try {
    await db.query('UPDATE tlv_suscriptores SET activo=false WHERE id=$1', [req.params.id]);
    res.json({ ok: true });
  } catch(e) { res.json({ ok: false, error: e.message }); }
});

app.post('/tlv/monitor', (req, res) => {
  if (!req.user) return res.status(401).json({ ok: false });
  const { activo } = req.body;
  tlvMonitorActivo = activo !== false;
  console.log(`[TLV] Monitor ${tlvMonitorActivo ? 'activado' : 'desactivado'}`);
  res.json({ ok: true, activo: tlvMonitorActivo });
});
app.get('/tlv/monitor', (req, res) => {
  if (!req.user) return res.status(401).json({ ok: false });
  res.json({ activo: tlvMonitorActivo });
});
app.post('/tlv/test', async (req, res) => {
  if (!req.user) return res.status(401).json({ ok: false });
  try { await cronMonitorTLV(); res.json({ ok: true }); }
  catch(e) { res.json({ ok: false, error: e.message }); }
});
...
function startCronJobs() {
  ...
  // Monitor TLV cada 3 minutos
  setInterval(cronMonitorTLV, 3 * 60 * 1000);
  ...
  console.log('[Cron] ✅ Auto-verify: cada 6hs | Check-in: cada 1h | Monitor TLV: cada 3min');
}
...
function generarPDFBuffer(opciones, vendedor, nombreCliente, contactoOverride = null) {
  return new Promise((resolve, reject) => {
    const NAVY = '#1B3A5C';
    const contacto = contactoOverride || CONTACTOS[vendedor] || CONTACTOS.guido;
    ...
    for (let i = 0; i < opciones.length; i++) {
      ...
      for (const pax of op.pasajeros) {
        const precio = pax.tipo_tarifa === 'CUSTOM' ? pax.neto : calcularPrecio(pax.neto, pax.tipo_tarifa, pax.comision_over);
        console.log(`[PDF-Precio] neto=${pax.neto} tipo=${pax.tipo_tarifa} → venta=${precio}`);
  ...
  try {
    ...
    const opcionesCompletas = await Promise.all(opciones.map(async (op) => {
      ...
      if (String(op.quotationId).startsWith('lleego_')) {
        ...
        // El cache ya tiene las asociaciones filtradas a la combinación correcta (un journey por tramo)
        const vuelos = [];
        const assocs = sol.data?.associations || [];
        for (const assoc of assocs) {
          const journeyRefs = assoc.journey_references || [];
    ...
    const cotizUserContacto = await getContactoUsuario(req.user?.userId);
    const pdfBuffer = await generarPDFBuffer(opcionesCompletas, vendedor || 'guido', nombreCliente || '', cotizUserContacto);
    ...
    // ── Nombre del PDF: "Cotizacion MAD TLV FCO 07-03" ──
    const hoy = new Date();
    const fechaStr = String(hoy.getDate()).padStart(2,'0') + '-' + String(hoy.getMonth()+1).padStart(2,'0');

    // Extraer destinos únicos de todos los vuelos de la primera opción
    // Lógica: sacar todos los destinos (IATA), quitar el primer origen y el último destino si vuelve al origen
    const todosVuelos = opcionesCompletas[0]?.vuelos || [];
    const iataRegex = /\(([A-Z]{3})\)/;
    const origenes = todosVuelos.map(v => { const m = (v.origen||'').match(iataRegex); return m ? m[1] : ''; }).filter(Boolean);
    const destinos = todosVuelos.map(v => { const m = (v.destino||'').match(iataRegex); return m ? m[1] : ''; }).filter(Boolean);
    const primerOrigen = origenes[0] || '';
    const ultimoDestino = destinos[destinos.length - 1] || '';
    // Construir lista de destinos sin repetir, sin el primer origen, sin el último si igual al primer origen
    const destinosSinRepetir = [];
    for (const d of destinos) {
      if (d === primerOrigen && d === ultimoDestino && destinosSinRepetir.length === destinos.length - 1) break; // ida y vuelta: skip último
      if (!destinosSinRepetir.includes(d)) destinosSinRepetir.push(d);
    }
    // Si el último destino es igual al primer origen (vuelta al mismo), quitarlo
    if (destinosSinRepetir[destinosSinRepetir.length - 1] === primerOrigen) {
      destinosSinRepetir.pop();
    }
    const nombrePDF = `Cotizacion ${destinosSinRepetir.join(' ')} ${fechaStr}`;

    res.set({ 'Content-Type': 'application/pdf', 'Content-Disposition': `attachment; filename="${nombrePDF}.pdf"` });

    // ── Enviar mail con PDF adjunto ──
    try {
      const cotizContacto = await getContactoUsuario(req.user?.userId);
      const cotizFrom = cotizContacto ? `${cotizContacto.nombre} <${cotizContacto.mail}>` : 'Lucky Tour <guido@luckytourviajes.com>';
      const cotizTo = cotizContacto?.mail || EMAIL_FROM;
      await resendClient.emails.send({
        from: cotizFrom,
        to: cotizTo,
        subject: nombrePDF,
      ...
      console.log(`[Cotizacion] Mail enviado: ${nombrePDF}`);
    } catch(mailErr) {
      console.error('[Cotizacion] Error mail:', mailErr.message);
    }
