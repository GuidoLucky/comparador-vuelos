...
// ─────────────────────────────────────────────────────────────
// MONITOR TLV — Alerta interna de disponibilidad Israel
// ─────────────────────────────────────────────────────────────
const TLV_DESTINOS = [
  { iata: 'FCO', nombre: 'Roma' }, { iata: 'MXP', nombre: 'Milán' },
  { iata: 'AMS', nombre: 'Amsterdam' }, { iata: 'MAD', nombre: 'Madrid' },
  { iata: 'BCN', nombre: 'Barcelona' }, { iata: 'FRA', nombre: 'Frankfurt' },
  { iata: 'CDG', nombre: 'París' }, { iata: 'LHR', nombre: 'Londres' },
  { iata: 'PRG', nombre: 'Praga' }, { iata: 'ATH', nombre: 'Atenas' },
  { iata: 'LIS', nombre: 'Lisboa' }, { iata: 'ZRH', nombre: 'Zurich' },
  { iata: 'GVA', nombre: 'Ginebra' },
];
const TLV_FECHAS = ['2026-03-10', '2026-03-11', '2026-03-12', '2026-03-15', '2026-03-16'];
const TLV_AEROLINEAS = ['IZ', 'IS', 'LY'];
let tlvMonitorActivo = true;

async function cronMonitorTLV() {
  const hoy = new Date().toISOString().substring(0, 10);
  if (hoy > '2026-03-16') { tlvMonitorActivo = false; return; }
  if (!tlvMonitorActivo) return;

  const combinaciones = [];
  for (const fecha of TLV_FECHAS) {
    if (fecha < hoy) continue;
    for (const dest of TLV_DESTINOS) combinaciones.push({ fecha, dest });
  }
  if (!combinaciones.length) { tlvMonitorActivo = false; return; }
  ...
  for (let i = 0; i < combinaciones.length; i += BATCH) {
    ...
    await Promise.all(batch.map(async ({ fecha, dest }) => {
      try {
        const lleegoToken = await getLleegoToken();
        const payload = {
          origin: 'TLV', destination: dest.iata,
          departure_date: fecha, adults: 1, children: 0, infants: 0,
          currency: 'USD', max_layover_count: 0
        };
        const lr = await fetch('https://api-tr.lleego.com/api/v2/transport/avail?locale=es-ar', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${lleegoToken}`, 'X-API-KEY': LLEEGO_API_KEY },
          body: JSON.stringify(payload)
        });
        const lData = lr.ok ? await lr.json() : null;
        const sols = lData?.solutions || [];
        console.log(`[TLV] GEA TLV→${dest.iata} ${fecha}: ${sols.length} soluciones totales`);
        if (!sols.length) return;

        // Usar procesarVuelosLleego para leer la estructura correcta, igual que la búsqueda normal
        const vuelosProcesados = procesarVuelosLleego(lData, { adultos: 1, ninos: 0, infantes: 0 });
        const filtGEA = vuelosProcesados.filter(v => {
          const ae = (v.aerolinea || v.aerolineaIda || '').toUpperCase();
          return TLV_AEROLINEAS.some(code => ae.includes(code));
        });
        console.log(`[TLV] GEA TLV→${dest.iata} ${fecha}: ${filtGEA.length} con IZ/IS/LY (aerolíneas: ${[...new Set(filtGEA.map(v => v.aerolinea || v.aerolineaIda || '?'))].join(', ')})`);

        if (!filtGEA.length) return;

        const vf = filtGEA[0];
        const aerolinea = vf.aerolinea || '';
        const it = vf.itinerario?.[0];
        const seg0 = it?.segmentos?.[0];
        const segLast = it?.segmentos?.[it.segmentos.length - 1];
        const vuelo = seg0 ? `${seg0.aerolinea || aerolinea}${seg0.vuelo || ''}` : aerolinea;
        const salida = seg0?.salida?.substring(11, 16) || it?.salida?.substring(11, 16) || '';
        const llegada = segLast?.llegada?.substring(11, 16) || it?.llegada?.substring(11, 16) || '';
        const precioFinal = Math.ceil((vf.precioUSD || 0) + 40);

        // Extraer seats desde la solución raw usando lleegoId
        let seats = null;
        const rawSol = (lData.solutions || []).find(s => s.id === vf.lleegoId);
        if (rawSol) {
          const rawAssocs = rawSol.data?.associations || [];
          for (const assoc of rawAssocs) {
            for (const segId of Object.keys(assoc.segment_references || {})) {
              const sr = assoc.segment_references[segId];
              if (sr?.seats != null && (seats === null || sr.seats < seats)) seats = sr.seats;
            }
          }
        }
        const fechaFmt = new Date(fecha + 'T12:00:00').toLocaleDateString('es-AR', { weekday: 'short', day: 'numeric', month: 'short' });
        const seatsStr = seats != null ? ` · ${seats} lugar${seats !== 1 ? 'es' : ''}` : '';
        const mensaje = `✈ TLV → ${dest.iata} (${dest.nombre}) ${fechaFmt} — ${vuelo} ${salida}→${llegada}${seatsStr} — USD ${precioFinal}`;
        console.log(`[TLV] ✅ ${mensaje}`);

        encontrados.push({ fecha, dest, mensaje, vuelo, salida, llegada, seats, precio: precioFinal, cantidad: filtGEA.length });
      } catch(e) {
  ...
  if (encontrados.length && db) {
    console.log(`[TLV] ${encontrados.length} rutas con disponibilidad — insertando notificaciones`);
    for (const e of encontrados) {
      try {
        // Evitar duplicados: no insertar si ya existe la misma notif no leída hoy
        ...
        await db.query(
          ...
          ['tlv_disponibilidad', e.mensaje, JSON.stringify({ fecha: e.fecha, destino: e.dest, vuelo: e.vuelo, salida: e.salida, llegada: e.llegada, seats: e.seats, precio: e.precio })]
        );
