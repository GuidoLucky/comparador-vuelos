...
// ─── TEMPORAL: Reset cron_finalizado (BORRAR DESPUÉS DE USAR) ───
app.get('/admin/reset-cron', async (req, res) => {
  try {
    const r = await db.query("UPDATE reservas SET cron_finalizado=false WHERE estado IN ('CREADA','EMITIDA')");
    console.log(`[Reset-Cron] ${r.rowCount} reservas reseteadas`);
    res.json({ ok: true, mensaje: `${r.rowCount} reservas reseteadas. Ahora andá a /cron/verificar` });
  } catch(e) { res.json({ ok: false, error: e.message }); }
});
        ...
        if (estadoCambio || hayCambios) {
          try {
            ...
            const notifMsg  = cambiosItinerario.length
              ? `${cambiosItinerario.length} cambio(s) detectado(s) en el itinerario`
              : mensaje || `Estado: ${apiEstado}`;
            const existing = await db.query(
              `SELECT id FROM notificaciones WHERE reserva_id=$1 AND tipo=$2 AND created_at > NOW() - INTERVAL '24 hours'`,
              [reserva.id, notifTipo]
            );
            if (!existing.rows.length || estadoCambio) {
              await db.query(`INSERT INTO notificaciones (reserva_id, pnr, tipo, mensaje, detalle) VALUES ($1,$2,$3,$4,$5)`,
                [reserva.id, reserva.pnr, notifTipo, notifMsg,
                 JSON.stringify({ cambios: todosCambios, alternativas, tipoEvento })]);
            }
          } catch(iaErr) {
...
function detectarCambiosItinerario(itinerarioGuardado, vuelosAPI) {
  ...
  for (let i = 0; i < vuelosAPI.length; i++) {
    const v = vuelosAPI[i];
    const g = segsGuardados[i];
    if (!g) continue;

    const flightNum = v.flightNumber || `${v.marketingAirlineCode || ''}${v.flightNumber || ''}`;
    const depAPI   = v.departureDateTime || v.departureDate || '';
    const arrAPI   = v.arrivalDateTime   || v.arrivalDate   || '';
    const depG     = g.salida || g.departureDateTime || '';
    const arrG     = g.llegada || g.arrivalDateTime   || '';
    const depIATA  = v.departureAirportCode || v.origin || '';
    const arrIATA  = v.arrivalAirportCode   || v.destination || '';
    const segLabel = `${flightNum || g.vuelo || ''} ${depIATA}→${arrIATA}`;
    ...
    if (depIATA && origG && depIATA !== origG) cambios.push({ tipo: 'AEROPUERTO', segmento: segLabel, campo: 'origen', original: origG, nuevo: depIATA });
    if (arrIATA && destG && arrIATA !== destG) cambios.push({ tipo: 'AEROPUERTO', segmento: segLabel, campo: 'destino', original: destG, nuevo: arrIATA });

    // Cambio de número de vuelo
    const vueloG = g.vuelo || g.flightNumber || '';
    if (flightNum && vueloG && flightNum.replace(/\s/g,'') !== vueloG.replace(/\s/g,'')) {
