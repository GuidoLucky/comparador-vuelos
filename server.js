...
async function enviarEmailReserva(reserva, pasajeros, paxPrecios = null) {
  ...
  try {
    const tipoMap = { adulto: 'Adulto', menor: 'Menor', 'bebé': 'Bebé', bebe: 'Bebé', adt: 'Adulto', chd: 'Menor', cnn: 'Menor', inf: 'Bebé' };
    const paxNombres = pasajeros.map(p => {
      const tipoRaw = (p.tipo || 'adulto').toLowerCase().replace('é','e');
      const tipoLabel = tipoMap[tipoRaw] || p.tipo || 'Adulto';
      return `${p.apellido || ''}, ${p.nombre || ''} <span style="background:#f0f0f0;padding:2px 8px;border-radius:10px;font-size:12px;color:#666;">${tipoLabel}</span>`;
    }).join('<br>');
    ...
    function buildPrecioVentaHtml(paxArray, esCliente = false) {
      ...
      const tipoLabel = { adulto: 'adulto', menor: 'menor', bebe: 'bebé', adt: 'adulto', chd: 'menor', cnn: 'menor', inf: 'bebé' };
      const grupos = {};
      ...
      for (const g of Object.values(grupos)) {
        ...
        const label = g.cantidad > 1
          ? `USD ${g.precio.toLocaleString('es-AR')} cada ${g.tipo}`
          : `USD ${g.precio.toLocaleString('es-AR')} ${g.tipo}`;
        const subtotal = g.precio * g.cantidad;
        rows += `<tr>
          <td style="padding:8px 12px;color:#444;"><strong>${label}</strong></td>
        </tr>`;
      }
      if (!rows) return '';
      return `
